...
bool write_objfile_per_region(const NonDestructiveTriMesh &mesh, const std::vector<Vec3d> &x, int label, const char *filename_format, ...)
{
   ...
   std::cout << "Contracting data for region: " << label << std::endl;
   std::vector<ElTopo::Vec3d> new_points;
   std::vector<ElTopo::Vec3st> new_tris;
   ...
   for(size_t i = 0; i < vert_needed.size(); ++i) {
      if(vert_needed[i]) {
         ...
         new_indices[i] = new_points.size()-1;
      }
      else 
         new_indices[i] = -1;
   ...
   output<<"# generated by VoronoiFluid3D - meeting all wacky fluid sim needs"<<std::endl;
