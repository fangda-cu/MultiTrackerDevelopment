...
const ElTopo::Array1d* ParseTree::
get_vector(const std::string& name) const
{
   std::map<std::string,ElTopo::Array1d>::const_iterator p=vectors.find(name);
   if(p==vectors.end()) return 0;
   else return &p->second;
}

bool ParseTree::
get_vec2d(const std::string& name, ElTopo::Vec2d& v) const
{
   std::map<std::string,ElTopo::Array1d>::const_iterator p=vectors.find(name);
...
bool ParseTree::
get_vec3d(const std::string& name, ElTopo::Vec3d& v) const
{
   std::map<std::string,ElTopo::Array1d>::const_iterator p=vectors.find(name);
   if(p==vectors.end()) return false;
   if(p->second.n!=3){
...
static void write_tree(std::ostream& out, const ParseTree& tree, int indentation)
{
   std::string indent_string;
   indent_string.assign(indentation, ' ');

   std::map<std::string,double>::const_iterator n;
   for(n=tree.numbers.begin(); n!=tree.numbers.end(); ++n)
      out<<indent_string<<n->first<<" = "<<n->second<<std::endl;

   std::map<std::string,std::string>::const_iterator s;
   for(s=tree.strings.begin(); s!=tree.strings.end(); ++s)
      out<<indent_string<<s->first<<" = ["<<s->second<<"]"<<std::endl;

   std::map<std::string,ElTopo::Array1d>::const_iterator v;
   for(v=tree.vectors.begin(); v!=tree.vectors.end(); ++v){
      out<<indent_string<<v->first<<" = [";
      const ElTopo::Array1d& a(v->second);
...
static bool parse_vector(Lexer& lexer, ElTopo::Array1d& v)
{
   Token token;
   v.resize(0);
   for(;;){
...
static bool recursive_parse(Lexer& lexer, ParseTree& tree, bool root_level)
{
   Token token;
   std::string name;
   for(;;){
