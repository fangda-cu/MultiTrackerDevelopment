...
public:
    ...
    explicit GroomingTimeStepper(ElasticRod& rod) :
        m_rod(rod), m_method(NONE), m_diffEqSolver(NULL)
    ...
    void getMass(VecXd& masses)
    {
        ...
        for (int i = 0; i < m_rod.ndof(); ++i)
            masses(i) = m_rod.getMass(i);
    }
    ...
    void evaluateConservativeForcesEnergy(VecXd& f, Scalar& energy)
    {
        ...
        for (size_t i = 0; i < m_externalForces.size(); ++i)
        {
            ...
            {
                RodExternalConservativeForce* potentialForce =
                        dynamic_cast<RodExternalConservativeForce*> (m_externalForces[i]);
    ...
    virtual Scalar get_stol() const
    {
        return m_diffEqSolver->get_stol();
    }
    virtual void set_stol(Scalar s)
    {
        m_diffEqSolver->set_stol(s);
    }

    virtual Scalar get_atol() const
    {
        return m_diffEqSolver->get_atol();
    }
    virtual void set_atol(Scalar a)
    {
        m_diffEqSolver->set_atol(a);
    }

    virtual Scalar get_rtol() const
    {
        return m_diffEqSolver->get_rtol();
    }
    virtual void set_rtol(Scalar r)
    {
        m_diffEqSolver->set_rtol(r);
    }

    virtual Scalar get_inftol() const
    {
        return m_diffEqSolver->get_inftol();
    }
    virtual void set_inftol(Scalar i)
    {
        m_diffEqSolver->set_inftol(i);
    }
    ...
    void getScriptedDofs(IntArray& indices, std::vector<Scalar>& desired)
    {
        ...
        const RodBoundaryCondition::BCList& verts = m_rod.getBoundaryCondition()->scriptedVertices();
        //= m_boundaryCondition->scriptedVertices();
        const RodBoundaryCondition::BCList& edges = m_rod.getBoundaryCondition()->scriptedEdges();
        ...
        for (size_t i = 0; i < verts.size(); ++i)
        {
            for (int k = 0; k < 3; ++k)
            {
                ...
                desired[3 * i + k] = m_rod.getBoundaryCondition()->getDesiredVertexPosition(verts[i], getTime())[k];
        ...
        for (size_t i = 0; i < edges.size(); ++i)
        {
            ...
            desired[3 * verts.size() + i] = m_rod.getBoundaryCondition()->getDesiredEdgeAngle(edges[i], getTime());
