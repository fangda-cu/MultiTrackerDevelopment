...
Vec3d RodBoundaryCondition::getDesiredVertexPosition(int vertIdx, double t)
{
    ...
    std::cout << "RodBoundaryCondition::getDesiredVertexPosition: t = " << t << std::endl;//<< " vertIdx = " << vertIdx << " t0 = " << t0 << " x0 = " << x0 << " v0 = " << v0 << " result = " << result << std::endl;
...
void RodBoundaryCondition::setDesiredEdgeAngle(int edgeIdx, double t, const Scalar& theta, const Scalar& thetaDot)
{
    ...
    BCList::iterator result = std::find(edges.begin(), edges.end(), edgeIdx);
    if (result == edges.end())
