...
void BridsonStepper::prepareForExecution()
{
  ...
  for( int i = 0; i < (int) m_triangle_meshes.size(); ++i )
  {
    ...
    for( int j = 0; j < m_triangle_meshes[i]->nv(); ++j ) m_vertex_radii.push_back(0.0);
    
    // Extract masses from the tri_mesh (just infinity for this object)
    for( int j = 0; j < m_triangle_meshes[i]->nv(); ++j ) m_masses.push_back(std::numeric_limits<double>::infinity());
  ...
  //std::cout << "About to create BVH" << std::endl;
...
void BridsonStepper::exertInelasticImpluse( EdgeEdgeContinuousTimeCollision& clssn )
{
  assert( clssn.e0_v0 >= 0 ); assert( clssn.e0_v0 < getNumVerts() );
  assert( clssn.e0_v1 >= 0 ); assert( clssn.e0_v1 < getNumVerts() );
  assert( clssn.e1_v0 >= 0 ); assert( clssn.e1_v0 < getNumVerts() );
  assert( clssn.e1_v1 >= 0 ); assert( clssn.e1_v1 < getNumVerts() );

  // Compute the relative velocity of the edges at the collision point
  Vec3d relvel = computeRelativeVelocity( m_vnphalf, clssn.e0_v0, clssn.e0_v1, 
                                          clssn.e1_v0, clssn.e1_v1, clssn.s, clssn.t );
  double magrelvel = relvel.dot(clssn.n);

  if( magrelvel >= 0.0 ) 
  { 
    std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Relative velocity computed \
                  incorrectly before applying edge-edge inelastic impulse (bug in normal \
                  computation?). Magnitude of relative velocity: " << magrelvel 
                  << ". Rod: " << getContainingRod(clssn.e0_v0) << std::endl;
  }
  assert( magrelvel < 0.0 );  

  // Add some extra "kick" to relative velocity to account for FPA errors
  magrelvel -= 1.0e-6;
  Vec3d I = computeEdgeEdgeInelasticImpulse( m_masses[clssn.e0_v0], m_masses[clssn.e0_v1], 
                                             m_masses[clssn.e1_v0], m_masses[clssn.e1_v1],
                                             clssn.s, clssn.t, magrelvel, clssn.n );
  
  exertEdgeImpulse( -I, m_masses[clssn.e0_v0], m_masses[clssn.e0_v1], clssn.s, clssn.e0_v0, clssn.e0_v1, m_vnphalf );
  exertEdgeImpulse(  I, m_masses[clssn.e1_v0], m_masses[clssn.e1_v1], clssn.t, clssn.e1_v0, clssn.e1_v1, m_vnphalf );
  
  assert( computeRelativeVelocity( m_vnphalf, clssn.e0_v0, clssn.e0_v1, clssn.e1_v0, clssn.e1_v1, clssn.s, clssn.t ).dot(clssn.n) >= 0.0 );  
}

void BridsonStepper::exertInelasticImpluse( VertexFaceContinuousTimeCollision& clssn )
{
  assert( clssn.v0 >= 0 ); assert( clssn.v0 < getNumVerts() );
  assert( clssn.f0 >= 0 ); assert( clssn.f0 < getNumVerts() );
  assert( clssn.f1 >= 0 ); assert( clssn.f1 < getNumVerts() );
  assert( clssn.f2 >= 0 ); assert( clssn.f2 < getNumVerts() );

  // Compute the relative velocity of the edges at the collision point
  Vec3d relvel = computeRelativeVelocity(m_vnphalf,clssn.v0,clssn.f0,clssn.f1,clssn.f2,clssn.u,clssn.v,clssn.w);
  double magrelvel = relvel.dot(clssn.n);

  if( magrelvel >= 0.0 ) 
  { 
    std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Relative velocity computed \
                  incorrectly before applying vertex-face inelastic impulse (bug in normal \
                  computation?). Magnitude of relative velocity: " << magrelvel << ". Rod: "
                  << getContainingRod(clssn.v0) << std::endl;
  }
  assert( magrelvel < 0.0 );

//    // TODO: Remove once done debugging the hairry ball.
//    {
//      // For this convex object centered at origin, normals dotted into a position on the triangle must be positive.
//      const Vec3d p  = m_xn.segment<3>(3*clssn.v0);
//      const Vec3d f0 = m_xn.segment<3>(3*clssn.f0);
//      const Vec3d f1 = m_xn.segment<3>(3*clssn.f1);
//      const Vec3d f2 = m_xn.segment<3>(3*clssn.f2);
//
//      const Vec3d vp  = m_vnphalf.segment<3>(3*clssn.v0);
//      const Vec3d vf0 = m_vnphalf.segment<3>(3*clssn.f0);
//      const Vec3d vf1 = m_vnphalf.segment<3>(3*clssn.f1);
//      const Vec3d vf2 = m_vnphalf.segment<3>(3*clssn.f2);
//
//      const Vec3d pcol  = p +clssn.time*m_dt*vp;
//      const Vec3d f0col = f0+clssn.time*m_dt*vf0;
//      const Vec3d f1col = f1+clssn.time*m_dt*vf1;
//      const Vec3d f2col = f2+clssn.time*m_dt*vf2;
//
//      if( clssn.n.dot( f0col ) <= 0.0 || clssn.n.dot( f1col ) <= 0.0 || clssn.n.dot( f2col ) <= 0.0 )
//      {
//        std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m \"Backside\" collision detected. \
//                     Probably not a big deal. Rod: " << getContainingRod(clssn.v0) << " Vertex: " 
//                     << clssn.v0 << " Time: " << m_t << std::endl;
//      }
//      //assert( clssn.n.dot( f0col ) > 0.0 );
//      //assert( clssn.n.dot( f1col ) > 0.0 );
//      //assert( clssn.n.dot( f2col ) > 0.0 );
//    }
    
  // Add some extra "kick" to relative velocity to account for FPA errors
  magrelvel -= 1.0e-6;
  Vec3d I = computeVertexFaceInelasticImpulse( m_masses[clssn.v0], m_masses[clssn.f0], 
                                               m_masses[clssn.f1], m_masses[clssn.f2],
                                               clssn.u, clssn.v, clssn.w, magrelvel, clssn.n );
  
  exertFaceImpulse( -I, m_masses[clssn.f0], m_masses[clssn.f1], m_masses[clssn.f2], 
                    clssn.u, clssn.v, clssn.w, clssn.f0, clssn.f1, clssn.f2, m_vnphalf );
  exertVertexImpulse( I, m_masses[clssn.v0], clssn.v0, m_vnphalf );
  
  assert( computeRelativeVelocity(m_vnphalf,clssn.v0,clssn.f0,clssn.f1,clssn.f2,clssn.u,clssn.v,clssn.w).dot(clssn.n) >= 0.0 );  
}

  
// TODO: Move this out of here!
bool compareEdgeEdgeCollisionTime( const EdgeEdgeContinuousTimeCollision& collisioni, const EdgeEdgeContinuousTimeCollision& collisionj )
{
  return collisioni.time < collisionj.time;
}

// TODO: Move this out of here!
bool compareVertexFaceCollisionTime( const VertexFaceContinuousTimeCollision& collisioni, const VertexFaceContinuousTimeCollision& collisionj )
{
  return collisioni.time < collisionj.time;
}

// TODO: Move this out of here!
bool compareCollisionTimes( const ContinuousTimeCollision& cllsnA, const ContinuousTimeCollision& cllsnB )
{
...
{
public:
  bool operator() ( const ContinuousTimeCollision& cllsnA, const ContinuousTimeCollision& cllsnB ) const
  {
    return cllsnA.getTime() < cllsnB.getTime();
  }
};
...
void BridsonStepper::filterCollisions( std::list<ContinuousTimeCollision>& cllsns )
{
  std::list<ContinuousTimeCollision>::iterator itr = cllsns.begin();
  while( itr != cllsns.end() )
  {
    bool happens = false;
    if( itr->isEdgeEdge() ) computeEdgeEdgeContinuousTimeInfo(itr->getEdgeEdge(),happens);
    else computeVertexFaceContinuousTimeInfo(itr->getVertexFace(),happens);
    if( !happens ) itr = cllsns.erase(itr);
    else ++itr;
  }  
}
...
void BridsonStepper::executeIterativeInelasticImpulseResponse()
{
  ...
  std::list<ContinuousTimeCollision> cllsns;
  m_bvh->getContinuousTimeCollisions(cllsns);
  filterCollisions(cllsns);
  ...
  if( itr == m_num_inlstc_itrns ) 
    std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Exceeded maximum " <<
                 "number of inelastic iterations " << m_num_inlstc_itrns << 
                 ". Time of warning " << m_t << "." << std::endl;
...
{
  assert( vert_idx >= 0 ); assert( vert_idx < getNumVerts() );
  int rod = -1;
...
bool BridsonStepper::execute()
{
  ...
  if( do_adaptive ) return adaptiveExecute(m_dt);
...
void BridsonStepper::setDt( double dt )
{
  ...
  for( int i = 0; i < (int) m_steppers.size(); ++i ) m_steppers[i]->setTimeStep(dt);
  
  // Set the timestep for the scripted object controllers
  for( int i = 0; i < (int) m_scripting_controllers.size(); ++i ) m_scripting_controllers[i]->setDt(dt);
}
...
bool BridsonStepper::nonAdaptiveExecute( double dt )
{
  setDt(dt);
  m_t += dt;
  for( int i = 0; i < (int) m_scripting_controllers.size(); ++i ) m_scripting_controllers[i]->setTime(m_t);
  return step(false);
}
...
bool BridsonStepper::adaptiveExecute( double dt )
{
  ...
  if( !first_success ) { setDt(dt); return false; }
  bool second_success = adaptiveExecute(0.5*dt);
  if( !second_success ) { setDt(dt); return false; }
...
bool BridsonStepper::step( bool check_explosion )
{
  ...
  if( m_rod_labels.size() != 0 ) assert( m_rod_labels.size() == m_rods.size() );
  ...
  if( check_explosion ) startforce = computeTotalForceNorm();
  ...
  for( int i = 0; i < (int) m_scripting_controllers.size(); ++i ) m_scripting_controllers[i]->execute();
  ...
  for( int i = 0; i < (int) m_steppers.size(); ++i )
  {
    if( !m_steppers[i]->execute() ) 
    {
      ...
      #endif
      if( m_rod_labels.size() == 0 ) std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Solver " << i << " failed at time " << m_t << "." << std::endl;
      else std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Solver " << m_rod_labels[i] << " failed at time " << m_t << "." << std::endl;
    }
  }
...
//  #endif
  ...
  if( m_itrv_inlstc_enbld && m_num_inlstc_itrns > 0 ) executeIterativeInelasticImpulseResponse();
...
//  #endif
  ...
  if( check_explosion )
  {
    ...
    if( endforce > startforce )
    {
      ...
      {
        if( endforce-startforce >= perrcent_change*startforce ) dependable_solve = false;
      }
      else 
      {
        if( endforce > big_force ) dependable_solve = false;
      }
    }
  }
...
void BridsonStepper::ensureCircularCrossSection( const ElasticRod& rod ) const
{
  ...
  for( int i = 0; i < (int) rod.ne(); ++i )
  {
    ...
    {
      std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Contact currently not supported for non-circular cross sections. Assuming circular cross section." << m_num_inlstc_itrns << std::endl;
    }
  }
}
...
void BridsonStepper::ensureNoCollisionsByDefault( const ElasticRod& rod ) const
{
  ...
  for( int i = 1; i < (int) rod.ne()-1; ++i )
  {
    ...
    if( edgelen <= radsum )
    {
      std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Detected edges that collide by default. Instabilities may result." << m_num_inlstc_itrns << std::endl;
    }
  }
}
...
void BridsonStepper::restorePositions( std::vector<ElasticRod*>& rods, const VecXd& positions )
{
  ...
  for( int i = 0; i < (int) m_base_indices.size(); ++i ) for( int j = 0; j < rods[i]->nv(); ++j )
      if( !m_rods[i]->getBoundaryCondition()->isVertexScripted(j) ) rods[i]->setVertex(j,positions.segment<3>(m_base_indices[i]+3*j));
}
...
void BridsonStepper::restoreVelocities( std::vector<ElasticRod*>& rods, const VecXd& velocities )
{
  ...
  for( int i = 0; i < (int) m_base_indices.size(); ++i ) for( int j = 0; j < rods[i]->nv(); ++j )
      if( !m_rods[i]->getBoundaryCondition()->isVertexScripted(j) ) rods[i]->setVelocity(j,velocities.segment<3>(m_base_indices[i]+3*j));
}

bool BridsonStepper::isRodVertex( int vert ) const
{
  assert( vert >= 0 ); assert( vert < getNumVerts() );
  
  // Is a vertex if index is less than start of object vertices in global array
  return vert < m_obj_start;
}

bool BridsonStepper::isRodRodCollision( const EdgeEdgeContinuousTimeCollision& collision ) const
{
  // Ensure and edge doesn't have vertices from a rod AND and triangulated object
  assert( (isRodVertex(collision.e0_v0) && isRodVertex(collision.e0_v1)) || (!isRodVertex(collision.e0_v0) && !isRodVertex(collision.e0_v1)) );
  assert( (isRodVertex(collision.e1_v0) && isRodVertex(collision.e1_v1)) || (!isRodVertex(collision.e1_v0) && !isRodVertex(collision.e1_v1)) );
  
  return isRodVertex(collision.e0_v0) && isRodVertex(collision.e0_v1) && isRodVertex(collision.e1_v0) && isRodVertex(collision.e1_v1);
}
...
{
  assert( v0 >= 0 ); assert( v0 < getNumVerts() );
  assert( v1 >= 0 ); assert( v1 < getNumVerts() );
  assert( v2 >= 0 ); assert( v2 < getNumVerts() );
...
{
  assert( e0v0 < getNumVerts() ); assert( e0v0 >= 0 );
  assert( e0v1 < getNumVerts() ); assert( e0v1 >= 0 );
  assert( e1v0 < getNumVerts() ); assert( e1v0 >= 0 );
  assert( e1v1 < getNumVerts() ); assert( e1v1 >= 0 );
...
bool BridsonStepper::vertexAndFaceShareVertex( const int& vertex, const int& face ) const
{
  ...
  assert( vertex >= 0 ); assert( vertex < getNumVerts() );
  assert( m_faces[face].idx[0] >= 0 ); assert( m_faces[face].idx[0] < getNumVerts() );
  assert( m_faces[face].idx[1] >= 0 ); assert( m_faces[face].idx[1] < getNumVerts() );
  assert( m_faces[face].idx[2] >= 0 ); assert( m_faces[face].idx[2] < getNumVerts() );
...
bool BridsonStepper::isProperCollisionTime( double time )
{
  if( time != time ) 
  { 
    if( !m_nan_enc ) std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Encountered NaN collision time from root finder. Supressing further messages of this type." << std::endl; 
    m_nan_enc = true;
    return false; 
  }
  if( time == std::numeric_limits<double>::infinity() ) 
  { 
    if( !m_inf_enc ) std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Encountered INF collision time from root finder. Supressing further messages of this type." << std::endl; 
    m_inf_enc = true;
    return false; 
  }
  if( time < 0.0 ) 
  {
    if( !m_lt0_enc ) std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Encountered scaled collision time " << time << " less than 0.0. Supressing further messages of this type." << std::endl; 
    m_lt0_enc = true;
    return false; 
  }
  if( time > 1.0 ) 
  {
    if( !m_gt0_enc ) std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Encountered scaled collision time " << time << " greater than 1.0. Supressing further messages of this type." << std::endl; 
    m_gt0_enc = true;
    return false; 
  }
  return true;
...
void BridsonStepper::computeEdgeEdgeContinuousTimeInfo( EdgeEdgeContinuousTimeCollision& eecol, bool& happens )
{
  assert( eecol.e0_v0 != eecol.e0_v1 ); assert( eecol.e1_v0 != eecol.e1_v1 );
  ...
  assert( eecol.e0_v0 >= 0 ); assert( eecol.e0_v0 < getNumVerts() );
  assert( eecol.e0_v1 >= 0 ); assert( eecol.e0_v1 < getNumVerts() );
  assert( eecol.e1_v0 >= 0 ); assert( eecol.e1_v0 < getNumVerts() );
  assert( eecol.e1_v1 >= 0 ); assert( eecol.e1_v1 < getNumVerts() );

  happens = false;
  
  // If edges are adjacent, do not register a collision
  if( edgesSharevertex(eecol.e0_v0,eecol.e0_v1,eecol.e1_v0,eecol.e1_v1) ) return;

  // Temporarily skip collisions involving a free edge and an edge with one fixed vertex
  //if( (isOneVertexFixed(eecol.e0_v0,eecol.e0_v1) && isEntireEdgeFree(eecol.e1_v0,eecol.e1_v1)) || (isOneVertexFixed(eecol.e1_v0,eecol.e1_v1) && isEntireEdgeFree(eecol.e0_v0,eecol.e0_v1)) ) 
  //{
  //  std::cout << "Temporarily skipping a fixed/free collision" << std::endl;
  //  return;
  //}
  
  // Temporarily skip rod 'self' collisions
  if( getContainingRod(eecol.e0_v0) == getContainingRod(eecol.e1_v0) )
  {
    //std::cout << "Temporarily skipping a rod colliding with itself" << std::endl;
    return;
  }

  // Temporarily skip rod-rod collisions
  if( SKIP_ROD_ROD ) if( isRodRodCollision( eecol ) ) return;

  const Vec3d& p1 = m_xn.segment<3>(3*eecol.e0_v0);
  const Vec3d& q1 = m_xn.segment<3>(3*eecol.e0_v1);
  const Vec3d& p2 = m_xn.segment<3>(3*eecol.e1_v0);
  const Vec3d& q2 = m_xn.segment<3>(3*eecol.e1_v1);

  const Vec3d& vp1 = m_vnphalf.segment<3>(3*eecol.e0_v0);
  const Vec3d& vq1 = m_vnphalf.segment<3>(3*eecol.e0_v1);
  const Vec3d& vp2 = m_vnphalf.segment<3>(3*eecol.e1_v0);
  const Vec3d& vq2 = m_vnphalf.segment<3>(3*eecol.e1_v1);
  ...
  {
    if( !isProperCollisionTime( times[j] ) ) continue;
    ...
    Vec3d c1;
    Vec3d c2;
    double sqrdist = ClosestPtSegmentSegment( p1col, q1col, p2col, q2col, eecol.s, eecol.t, c1, c2 );
    assert( sqrdist >= 0.0 );
    assert( eecol.s >= 0.0 ); assert( eecol.s <= 1.0 );
    assert( eecol.t >= 0.0 ); assert( eecol.t <= 1.0 );
    ...
    if( sqrdist < 1.0e-12 )
    {
      ...
      eecol.n = (q1col-p1col).cross(q2col-p2col);
      double nnorm = eecol.n.norm();
      ...
      if( nnorm == 0.0 )
      {
        ...
        eecol.n = ((1.0-eecol.t)*p2+eecol.t*q2).cross((1.0-eecol.s)*p1+eecol.s*q1);
        nnorm = eecol.n.norm();
      ...
      eecol.n /= nnorm;
      assert( approxEq(eecol.n.norm(),1.0,1.0e-6) );
      ...
      if( eecol.n.dot(relvel) > 0.0 ) eecol.n *= -1.0;

//      if( (getContainingRod(eecol.e0_v0) == 20 || getContainingRod(eecol.e1_v0) == 20 ) && m_t >= 0.85 )
//      {
//        // In this case, one fixed, one not
//        //assert( ((isEntireEdgeFixed(eecol.e0_v0,eecol.e0_v1) && isEntireEdgeFree(eecol.e1_v0,eecol.e1_v1)) || 
//        //        (isEntireEdgeFree(eecol.e0_v0,eecol.e0_v1) && isEntireEdgeFixed(eecol.e1_v0,eecol.e1_v1))) );
//        if( !((isEntireEdgeFixed(eecol.e0_v0,eecol.e0_v1) && isEntireEdgeFree(eecol.e1_v0,eecol.e1_v1)) || 
//              (isEntireEdgeFree(eecol.e0_v0,eecol.e0_v1) && isEntireEdgeFixed(eecol.e1_v0,eecol.e1_v1))) )
//        {
//          happens = true;
//          return;
//          std::cout << "Exit uh huh" << std::endl;
//          exit(1);
//        }
//        if( isEntireEdgeFree(eecol.e0_v0,eecol.e0_v1) ) std::cout << "Edge 0 is free" << std::endl;
//        if( isEntireEdgeFree(eecol.e1_v0,eecol.e1_v1) ) std::cout << "Edge 1 is free" << std::endl;
//
//        std::cout << "Rod 0: " << getContainingRod(eecol.e0_v0) << std::endl;
//        std::cout << "Rod 1: " << getContainingRod(eecol.e1_v0) << std::endl;
//        
//        std::cout << "Pre collision positions" << std::endl;
//        std::cout << "  p0 = " << p1.transpose() << "[[1]];" << std::endl;
//        std::cout << "  q0 = " << q1.transpose() << "[[1]];" << std::endl;
//        std::cout << "  p1 = " << p2.transpose() << "[[1]];" << std::endl;
//        std::cout << "  q1 = " << q2.transpose() << "[[1]];" << std::endl;
//        
//        std::cout << "At collision positions" << std::endl;
//        std::cout << "  atp0 = " << p1col.transpose() << "[[1]];" << std::endl;
//        std::cout << "  atq0 = " << q1col.transpose() << "[[1]];" << std::endl;
//        std::cout << "  atp1 = " << p2col.transpose() << "[[1]];" << std::endl;
//        std::cout << "  atq1 = " << q2col.transpose() << "[[1]];" << std::endl;        
//
//        std::cout << "Post collision positions" << std::endl;
//        std::cout << "  postp0 = " << (p1+m_dt*vp1).transpose() << "[[1]];" << std::endl;
//        std::cout << "  postq0 = " << (q1+m_dt*vq1).transpose() << "[[1]];" << std::endl;
//        std::cout << "  postp1 = " << (p2+m_dt*vp2).transpose() << "[[1]];" << std::endl;
//        std::cout << "  postq1 = " << (q2+m_dt*vq2).transpose() << "[[1]];" << std::endl;
//        
//        std::cout << "n = " << eecol.n.transpose() << "[[1]];" << std::endl;
//        
//        std::cout << "Rooted collision vector: " << std::endl;
//        std::cout << "  c1 = " << ((1-eecol.s)*p1col+eecol.s*q1col).transpose() << "[[1]];" << std::endl;
//        std::cout << "  c2 = " << ((1-eecol.s)*p1col+eecol.s*q1col+eecol.n).transpose() << "[[1]];" << std::endl;
//        
//        std::cout << "Collision time: " << times[j] << " scaled to " << dtime << std::endl;
//        
//        std::cout << "Magnitude rel-vel: " << eecol.n.dot(relvel) << std::endl;
...
void BridsonStepper::computeVertexFaceContinuousTimeInfo( VertexFaceContinuousTimeCollision& vfcol, bool& happens )
{
  assert( vfcol.v0 >= 0 ); assert( vfcol.v0 < getNumVerts() ); 
  assert( vfcol.f0 >= 0 ); assert( vfcol.f0 < getNumVerts() );
  assert( vfcol.f1 >= 0 ); assert( vfcol.f1 < getNumVerts() ); 
  assert( vfcol.f2 >= 0 ); assert( vfcol.f2 < getNumVerts() );
  assert( !vertexAndFaceShareVertex(vfcol.v0,vfcol.f0,vfcol.f1,vfcol.f2) );
  assert( vfcol.f0 != vfcol.f1 ); assert( vfcol.f0 != vfcol.f2 ); assert( vfcol.f1 != vfcol.f2 );
  
  happens = false;
  
  // In all current simulations, faces are scripted
  assert( isEntireFaceFixed(vfcol.f0,vfcol.f1,vfcol.f2) );

  // If vertex is fixed, if face is fixed, nothing to do
  if( isVertexFixed(vfcol.v0) && isEntireFaceFixed(vfcol.f0,vfcol.f1,vfcol.f2) ) return;

  const Vec3d& p  = m_xn.segment<3>(3*vfcol.v0);
  const Vec3d& f0 = m_xn.segment<3>(3*vfcol.f0);
  const Vec3d& f1 = m_xn.segment<3>(3*vfcol.f1);
  const Vec3d& f2 = m_xn.segment<3>(3*vfcol.f2);

  const Vec3d& vp  = m_vnphalf.segment<3>(3*vfcol.v0);
  const Vec3d& vf0 = m_vnphalf.segment<3>(3*vfcol.f0);
  const Vec3d& vf1 = m_vnphalf.segment<3>(3*vfcol.f1);
  const Vec3d& vf2 = m_vnphalf.segment<3>(3*vfcol.f2);
  ...
  // TODO: Delete this after more testing. Triangle objects are not deformable (yet), so area shouldn't change. Eventually we'll add deformable tri meshes (cloth).
  assert( approxEq((f1-f0).cross(f2-f0).norm(),((f1+m_dt*vf1)-(f0+m_dt*vf0)).cross((f2+m_dt*vf2)-(f0+m_dt*vf0)).norm(),1.0e-6) );
  ...
  for( size_t j = 0; j < times.size(); ++j )
  {
    ...
    if( (pcol-cp).squaredNorm() < 1.0e-12 )
    {
      ...
      vfcol.n = (f2col-f0col).cross(f1col-f0col);
      double nnorm = vfcol.n.norm();
      ...
      if( nnorm == 0.0 )
      {
        bool lazy_person_implemented_this_normal_computation = false;
        if( !lazy_person_implemented_this_normal_computation ) 
          std::cerr << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m YOU HIT AN UNSOPPORTED CODE PATH. REALLY DEGENERATE." << std::endl;
        assert( lazy_person_implemented_this_normal_computation );
        exit(1);
      }

      vfcol.n /= nnorm;
      assert( fabs(vfcol.n.norm()-1.0) < 1.0e-6 );

      Barycentric( f0col, f1col, f2col, pcol, vfcol.u, vfcol.v, vfcol.w );
      // Barycentric coords could be outside of [0,1] right now because we've extended the triangles a little bit
      assert( approxEq(vfcol.u+vfcol.v+vfcol.w,1.0) );

      Vec3d relvel = computeRelativeVelocity( m_vnphalf, vfcol.v0, vfcol.f0, vfcol.f1, vfcol.f2, vfcol.u, vfcol.v, vfcol.w );
      if( vfcol.n.dot(relvel) > 0.0 ) vfcol.n *= -1.0;
...
Vec3d BridsonStepper::computeRelativeVelocity( const VecXd& v, const int& idxa0, const int& idxa1, const int& idxb0, const int& idxb1, const double& s, const double& t )
{
  ...
  assert( idxa0 >= 0 ); assert( idxa0 < getNumVerts() ); 
  assert( idxa1 >= 0 ); assert( idxa1 < getNumVerts() );
  assert( idxb0 >= 0 ); assert( idxb0 < getNumVerts() ); 
  assert( idxb1 >= 0 ); assert( idxb1 < getNumVerts() );
...
Vec3d BridsonStepper::computeRelativeVelocity( const VecXd& vel, const int& vrtidx, const int& fcidx0, const int& fcidx1, const int& fcidx2, const double& u, const double& v, const double& w )
{
  ...
  assert( vrtidx >= 0 ); assert( vrtidx < getNumVerts() );
  assert( fcidx0 >= 0 ); assert( fcidx0 < getNumVerts() );
  assert( fcidx1 >= 0 ); assert( fcidx1 < getNumVerts() );
  assert( fcidx2 >= 0 ); assert( fcidx2 < getNumVerts() );
...
{
  assert( m > 0.0 );
  assert( idx >= 0 ); assert( idx < getNumVerts() );
  assert( v.size() == getNumDof() );
...
void BridsonStepper::exertEdgeImpulse( const Vec3d& I, const double& m0, const double& m1, const double& alpha, const int& idx0, const int& idx1, VecXd& v )
{
  ...
  assert( idx0 >= 0 ); assert( idx0 < getNumVerts() ); 
  assert( idx1 >= 0 ); assert( idx1 < getNumVerts() );
  assert( v.size() == getNumDof() );
                                       ...
                                       const int& idx0, const int& idx1, const int& idx2, VecXd& vel )
{
  ...
  assert( idx0 >= 0 ); assert( idx0 < getNumVerts() );
  assert( idx1 >= 0 ); assert( idx1 < getNumVerts() );
  assert( idx2 >= 0 ); assert( idx2 < getNumVerts() );
  assert( vel.size() == getNumDof() );
...
{
  assert( lhs != NULL );
  assert( rodidx >= 0 ); assert( rodidx < (int) m_rods.size() );
  ...
  // lhs = M - h^2*dF/dx
  for (int i = 0; i < m_rods[rodidx]->ndof(); ++i) lhs->add(i,i,m_rods[rodidx]->getMass(i));
  lhs->finalize();
...
void BridsonStepper::exertCompliantInelasticVertexFaceImpulse( const VertexFaceContinuousTimeCollision& vfcol )
{
  ...
  assert( v0 >= 0 ); assert( v0 < m_rods[rodidx]->nv() );
  
  // Compute the realtive velocity of the collision
  Vec3d relvel = computeRelativeVelocity(m_vnphalf,vfcol.v0,vfcol.f0,vfcol.f1,vfcol.f2,vfcol.u,vfcol.v,vfcol.w);
  double magrelvel = relvel.dot(vfcol.n);
  assert( magrelvel < 0.0 );
  ...
  n.back().setZero();
  n.back().segment<3>(base0) = vfcol.n;
  ...
  #ifdef DEBUG
    for( int i = 0; i < numconstraints; ++i ) assert( approxEq(n[i].norm(),1.0,1.0e-9) );
  #endif
  ...
  for( int i = 0; i < numconstraints; ++ i )
  {
    ...
    if( status < 0 ) std::cerr << "\033[31;1mWARNING IN IMPLICITEULER:\033[m Problem during linear solve detected. exertCompliantInelasticVertexFaceImpulse. Time: " << m_t << std::endl;
  }
  ...
  for( int i = 0; i < numconstraints; ++i ) posnn.push_back(VecXd(nvdof));
  #ifdef DEBUG
    for( int i = 0; i < numconstraints; ++i ) posnn[i].setConstant(std::numeric_limits<double>::signaling_NaN());
  #endif
  
  std::vector<VecXd> posnntilde;
  for( int i = 0; i < numconstraints; ++i ) posnntilde.push_back(VecXd(nvdof));
  #ifdef DEBUG
  ...
  #endif  
  for( int i = 0; i < numconstraints; ++i ) for( int j = 0; j < numconstraints; ++j ) lglhs(i,j) = posnn[i].dot(posnntilde[j]);
  ...
  assert( lgrhs(0) < 0.0 );
  for( int i = 1; i < numconstraints; ++i ) lgrhs(i) = posnn[i].dot(m_vnphalf.segment(rodbase,nvdof));
  lgrhs *= -1.0;
  ...
  for( int i = 0; i < numconstraints; ++i ) m_vnphalf.segment(rodbase,nvdof) += alpha(i)*posnntilde[i];
  ...
  // Ensure that the impulse eliminated the realtive velocity
    ...
    double postmagrelvel = postrelvel.dot(vfcol.n);
...
void BridsonStepper::exertCompliantInelasticEdgeEdgeImpulse( const EdgeEdgeContinuousTimeCollision& eecol )
{
  // Determine if either edge is totally fixed
  bool rod0fixed = isEntireEdgeFixed(eecol.e0_v0,eecol.e0_v1);
  bool rod1fixed = isEntireEdgeFixed(eecol.e1_v0,eecol.e1_v1);;

  // Skip collisions in which both edges are fixed
  if( rod0fixed && rod1fixed ) return;
  // Neither rod is totally fixed
  else if( !rod0fixed && !rod1fixed ) exertCompliantInelasticEdgeEdgeImpulseBothFree( eecol );
  // One rod is totally fixed
  else exertCompliantInelasticEdgeEdgeImpulseOneFixed( eecol );
}

void BridsonStepper::exertCompliantInelasticEdgeEdgeImpulseBothFree( const EdgeEdgeContinuousTimeCollision& eecol )
{
  ...
  assert( i0 >= 0 ); assert( i0 < m_rods[rod0]->nv() );
  int i1 = eecol.e0_v1 - rod0base/3;
  assert( i1 >= 0 ); assert( i1 < m_rods[rod0]->nv() );
  int j0 = eecol.e1_v0 - rod1base/3;
  assert( j0 >= 0 ); assert( j0 < m_rods[rod1]->nv() );
  int j1 = eecol.e1_v1 - rod1base/3;
  assert( j1 >= 0 ); assert( j1 < m_rods[rod1]->nv() );

  // Base indices within each rod's storage
  int base_i0 = m_rods[rod0]->vertIdx(i0,0);
  assert( base_i0 >= 0 ); assert( base_i0 < rod0ndof );
  int base_i1 = m_rods[rod0]->vertIdx(i1,0);
  assert( base_i1 >= 0 ); assert( base_i1 < rod0ndof );
  int base_j0 = m_rods[rod1]->vertIdx(j0,0);
  assert( base_j0 >= 0 ); assert( base_j0 < rod1ndof );
  int base_j1 = m_rods[rod1]->vertIdx(j1,0);
  assert( base_j1 >= 0 ); assert( base_j1 < rod1ndof );
  ...
  n0.back().setZero();
  n0.back().segment<3>(base_i0) = -(1.0-eecol.s)*eecol.n;
  n0.back().segment<3>(base_i1) = -eecol.s*eecol.n;
  ...
  n1.back().setZero();
  n1.back().segment<3>(base_j0) = (1.0-eecol.t)*eecol.n;
  n1.back().segment<3>(base_j1) = eecol.t*eecol.n;
  // 'Scripted vertex' constraints
  ...
  assert( lhs0 != NULL );
  assert( lhs0->rows() == rod0ndof ); assert( lhs0->cols() == rod0ndof );
  ...
  assert( lhs1->rows() == rod1ndof ); assert( lhs1->cols() == rod1ndof );
  assert( lhs1 != NULL );
  ...
  for( int i = 0; i < nc0; ++i ) posnn0.push_back(VecXd(rod0nvdof));
  #ifdef DEBUG
    for( int i = 0; i < nc0; ++i ) posnn0[i].setConstant(std::numeric_limits<double>::signaling_NaN());
  #endif  
  
  std::vector<VecXd> posnntilde0;
  for( int i = 0; i < nc0; ++i ) posnntilde0.push_back(VecXd(rod0nvdof));
  #ifdef DEBUG
  ...
  for( int i = 0; i < nc1; ++i ) posnn1.push_back(VecXd(rod1nvdof));
  #ifdef DEBUG
    for( int i = 0; i < nc1; ++i ) posnn1[i].setConstant(std::numeric_limits<double>::signaling_NaN());
  #endif
  
  std::vector<VecXd> posnntilde1;
  for( int i = 0; i < nc1; ++i ) posnntilde1.push_back(VecXd(rod1nvdof));
  #ifdef DEBUG
  ...
  int curel = 1;
  for( int i = 1; i < nc0; ++i ) lglhs(0,curel++) = posnn0[0].dot(posnntilde0[i]);
  // Entries (0,numalpha::numalpha+numbeta)
  for( int i = 1; i < nc1; ++i ) lglhs(0,curel++) = posnn1[0].dot(posnntilde1[i]);

  // Entires (1::numalpha,0)
  curel = 1;
  for( int i = 1; i < nc0; ++i ) lglhs(curel++,0) = posnntilde0[0].dot(posnn0[i]);
  // Entries (numalpha::numalpha+numbeta,0)
  for( int i = 1; i < nc1; ++i ) lglhs(curel++,0) = posnntilde1[0].dot(posnn1[i]);

  // Entries (1::numalpha,1::numalpha)
  for( int i = 1; i < nc0; ++i ) for( int j = 1; j < nc0; ++j ) lglhs(i,j) = posnn0[i].dot(posnntilde0[j]);
  // Entries (numalpha::numalpha+numbeta,numalpha::numalpha+numbeta)
  for( int i = 1; i < nc1; ++i ) for( int j = 1; j < nc1; ++j ) lglhs(nc0+i-1,nc0+j-1) = posnn1[i].dot(posnntilde1[j]);
  ...
  if( alpha(0) < 0.0 ) std::cout << "WARNING NEGATIVE LAGRANGE MULTIPLIER alpha: " << alpha << std::endl;
...
void BridsonStepper::exertCompliantInelasticEdgeEdgeImpulseOneFixed( const EdgeEdgeContinuousTimeCollision& eecol )
{
  // Must have one totally fixed and one totally free edge
  assert( (isEntireEdgeFree(eecol.e0_v0,eecol.e0_v1) && isEntireEdgeFixed(eecol.e1_v0,eecol.e1_v1)) || (isEntireEdgeFree(eecol.e1_v0,eecol.e1_v1) && isEntireEdgeFixed(eecol.e0_v0,eecol.e0_v1)) );
  ...
  assert( v0 >= 0 ); assert( v0 < m_rods[rodidx]->nv() );
  assert( v1 >= 0 ); assert( v1 < m_rods[rodidx]->nv() );
  ...
  n.push_back(VecXd(ndof));
  n.back().setZero();
  n.back().segment<3>(base0) = (1-u)*eecol.n;
  n.back().segment<3>(base1) = u*eecol.n;
  ...
  for( int i = 0; i < numconstraints; ++ i )
  {
    ...
    ntilde.back().setZero();    
    int status = solver->solve(ntilde.back(),n[i]);
    if( status < 0 ) std::cerr << "\033[31;1mWARNING IN IMPLICITEULER:\033[m Problem during linear solve detected. exertCompliantInelasticEdgeEdgeImpulseOneFixedThree. Time: " << m_t << std::endl;
  }
...
void BridsonStepper::detectVertexFaceImplicitPenaltyCollisions( const VecXd& x, std::vector<VertexFaceProximityCollision>& pssbl_cllsns, std::vector<VertexFaceImplicitPenaltyCollision>& vetex_face_collisions ) const
{
  assert( (int) m_masses.size() == getNumVerts() );
  ...
  for( int i = 0; i < (int) pssbl_cllsns.size(); ++i )
  {
    ...
    if( sqrdist < (m_implicit_thickness + pssbl_cllsns[i].r0+pssbl_cllsns[i].r1)*(m_implicit_thickness + pssbl_cllsns[i].r0+pssbl_cllsns[i].r1) )
    {
...
void BridsonStepper::executeImplicitPenaltyResponse()
{
  ...
  m_bvh->getImplicitPenaltyCollisions(edge_edge_collisions,vertex_face_collisions);
