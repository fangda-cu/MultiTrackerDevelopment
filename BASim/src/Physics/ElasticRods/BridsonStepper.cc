...
BridsonStepper::BridsonStepper(std::vector<ElasticRod*>& rods, std::vector<TriangleMesh*>& trimeshes,
	...
	m_num_dof(0), m_rods(rods), m_triangle_meshes(trimeshes), m_scripting_controllers(scripting_controllers),
			...
			m_vertex_radii(), m_edge_radii(), m_face_radii(), m_masses(), m_respns_enbld(true), m_pnlty_enbld(true),
			m_itrv_inlstc_enbld(true), m_num_inlstc_itrns(10), m_vrt_fc_pnlty(200.0), m_nan_enc(false), m_inf_enc(false),
			m_lt0_enc(false), m_gt0_enc(false), m_collision_detector(NULL), m_bvh(NULL), m_obj_start(-1), m_t(time),
			m_rod_labels(), m_implicit_pnlty_enbld(false), m_implicit_thickness(1.0)
{
#ifdef DEBUG
...
#ifdef BREANNAN_ORIGINAL
	std::cout << "About to create BVHAABB" << std::endl;
...
{
	assert(clssn.e0_v0 >= 0);
	assert(clssn.e0_v0 < getNumVerts());
	assert(clssn.e0_v1 >= 0);
	assert(clssn.e0_v1 < getNumVerts());
	assert(clssn.e1_v0 >= 0);
	assert(clssn.e1_v0 < getNumVerts());
	assert(clssn.e1_v1 >= 0);
	assert(clssn.e1_v1 < getNumVerts());

	// Compute the relative velocity of the edges at the collision point
	Vec3d relvel = computeRelativeVelocity(m_vnphalf, clssn.e0_v0, clssn.e0_v1, clssn.e1_v0, clssn.e1_v1, clssn.s, clssn.t);
	...
	if (magrelvel >= 0.0)
	{
		std::cerr
				...
				<< magrelvel << ". Rod: " << getContainingRod(clssn.e0_v0) << std::endl;
	}
	assert(magrelvel < 0.0);

	// Add some extra "kick" to relative velocity to account for FPA errors
	magrelvel -= 1.0e-6;
	Vec3d I = computeEdgeEdgeInelasticImpulse(m_masses[clssn.e0_v0], m_masses[clssn.e0_v1], m_masses[clssn.e1_v0],
			m_masses[clssn.e1_v1], clssn.s, clssn.t, magrelvel, clssn.n);
	...
	assert(
			computeRelativeVelocity(m_vnphalf, clssn.e0_v0, clssn.e0_v1, clssn.e1_v0, clssn.e1_v1, clssn.s, clssn.t).dot(
					clssn.n) >= 0.0);
}
...
{
	assert(clssn.v0 >= 0);
	assert(clssn.v0 < getNumVerts());
	assert(clssn.f0 >= 0);
	assert(clssn.f0 < getNumVerts());
	assert(clssn.f1 >= 0);
	assert(clssn.f1 < getNumVerts());
	assert(clssn.f2 >= 0);
	assert(clssn.f2 < getNumVerts());
	...
	if (magrelvel >= 0.0)
	{
		std::cerr
				<< "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Relative velocity computed \
                  ...
                  computation?). Magnitude of relative velocity: "
				<< magrelvel << ". Rod: " << getContainingRod(clssn.v0) << std::endl;
	}
	assert(magrelvel < 0.0);
...
//                     Probably not a big deal. Rod: " << getContainingRod(clssn.v0) << " Vertex: " 
	...
	// Add some extra "kick" to relative velocity to account for FPA errors
	magrelvel -= 1.0e-6;
	Vec3d I = computeVertexFaceInelasticImpulse(m_masses[clssn.v0], m_masses[clssn.f0], m_masses[clssn.f1], m_masses[clssn.f2],
			clssn.u, clssn.v, clssn.w, magrelvel, clssn.n);

	exertFaceImpulse(-I, m_masses[clssn.f0], m_masses[clssn.f1], m_masses[clssn.f2], clssn.u, clssn.v, clssn.w, clssn.f0,
			clssn.f1, clssn.f2, m_vnphalf);
...
bool compareEdgeEdgeCollisionTime(const EdgeEdgeContinuousTimeCollision& collisioni,
		const EdgeEdgeContinuousTimeCollision& collisionj)
{
	return collisioni.time < collisionj.time;
}

// TODO: Move this out of here!
bool compareVertexFaceCollisionTime(const VertexFaceContinuousTimeCollision& collisioni,
		const VertexFaceContinuousTimeCollision& collisionj)
{
	return collisioni.time < collisionj.time;
}
...
void BridsonStepper::filterCollisions(std::list<ContinuousTimeCollision>& cllsns)
{
	...
	std::cerr << "We have " << cllsns.size() << " candidates for collision" << std::endl;
	int DEBUG_edgedg = 0;
	int DEBUG_vtxfce = 0;
	while (itr != cllsns.end())
	{
		...
		if (itr->isEdgeEdge())
		{
			computeEdgeEdgeContinuousTimeInfo(itr->getEdgeEdge(), happens);
			if (happens)
				DEBUG_edgedg++;
		}
		else
		{
			computeVertexFaceContinuousTimeInfo(itr->getVertexFace(), happens);
			if (happens)
				DEBUG_vtxfce++;
		}
		if (!happens)
			itr = cllsns.erase(itr);
		else
			++itr;
	}
...
void BridsonStepper::executeIterativeInelasticImpulseResponse()
{
	...
	std::cerr << "Collision detector found " << cllsns.size() << " collisions" << std::endl;
#endif
...
{
	assert(eecol.e0_v0 != eecol.e0_v1);
	assert(eecol.e1_v0 != eecol.e1_v1);
	...
	if (edgesSharevertex(eecol.e0_v0, eecol.e0_v1, eecol.e1_v0, eecol.e1_v1))
		return;
	...
	for (size_t j = 0; j < times.size(); ++j)
	{
		...
		if (sqrdist < 1.0e-12)
		{
			...
			Vec3d relvel = computeRelativeVelocity(m_vnphalf, eecol.e0_v0, eecol.e0_v1, eecol.e1_v0, eecol.e1_v1, eecol.s,
					eecol.t);
			if (eecol.n.dot(relvel) > 0.0)
				eecol.n *= -1.0;
...
void BridsonStepper::computeVertexFaceContinuousTimeInfo(VertexFaceContinuousTimeCollision& vfcol, bool& happens)
{
	...
	assert(vfcol.f0 != vfcol.f1);
	assert(vfcol.f0 != vfcol.f2);
	assert(vfcol.f1 != vfcol.f2);
	...
	if (isVertexFixed(vfcol.v0) && isEntireFaceFixed(vfcol.f0, vfcol.f1, vfcol.f2))
		return;
	...
	assert(
			approxEq((f1 - f0).cross(f2 - f0).norm(),
					((f1 + m_dt * vf1) - (f0 + m_dt * vf0)).cross((f2 + m_dt * vf2) - (f0 + m_dt * vf0)).norm(), 1.0e-6));
	...
	for (size_t j = 0; j < times.size(); ++j)
	{
		...
		if ((pcol - cp).squaredNorm() < 1.0e-12)
		{
			...
			Vec3d relvel =
					computeRelativeVelocity(m_vnphalf, vfcol.v0, vfcol.f0, vfcol.f1, vfcol.f2, vfcol.u, vfcol.v, vfcol.w);
			if (vfcol.n.dot(relvel) > 0.0)
				vfcol.n *= -1.0;
...
Vec3d BridsonStepper::computeRelativeVelocity(const VecXd& v, const int& idxa0, const int& idxa1, const int& idxb0,
		const int& idxb1, const double& s, const double& t)
{
	assert(v.size() == getNumDof());
	assert(idxa0 >= 0);
	assert(idxa0 < getNumVerts());
	assert(idxa1 >= 0);
	assert(idxa1 < getNumVerts());
	assert(idxb0 >= 0);
	assert(idxb0 < getNumVerts());
	assert(idxb1 >= 0);
	assert(idxb1 < getNumVerts());
...
Vec3d BridsonStepper::computeRelativeVelocity(const VecXd& vel, const int& vrtidx, const int& fcidx0, const int& fcidx1,
