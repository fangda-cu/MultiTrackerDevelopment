...
void RodTwistingForceSym::updateProperties()
{
 ...
 //   int vtx = 1;
    for ( m_stencil = m_stencil.begin(); m_stencil != end; ++m_stencil )
    {
       ...
       // std::cout << "Computing twist for vertex " << vtx << ": ref = " << m_rod.getReferenceTwist(
        //        vh ) << " theta = " << m_rod.getTheta( eh1 ) << " previous theta = "
        //        << m_rod.getTheta( eh0 ) << " total = " << m_rod.getReferenceTwist( vh )
...
{
    const Scalar value = getKt( vh ) / getRefVertexLength( vh ) * ( getTwist( vh )
            - getUndeformedTwist( vh ) );
...
{
    const Scalar kt = getKt( vh );// std::cout << "kt = " << kt << '\n';
    const Scalar milen = -1.0 / getRefVertexLength( vh );// std::cout << "ilen = " << -milen << '\n';
    const Scalar twist = getTwist( vh );// std::cout << "twist = " << twist << '\n';
    const Scalar undeformedTwist = getUndeformedTwist( vh );// std::cout << "undefTwist = " << undeformedTwist << '\n';

    const ElementForce& gradTwist = getGradTwist( vh );// std::cout << "gradTwist = " << gradTwist << '\n';
...
void RodTwistingForceSym::localJacobianForceEnergy( MatXd& J, VecXd& force, Scalar& energy,
...
const RodTwistingForceSym::ElementJacobian& RodTwistingForceSym::getHessTwist(
