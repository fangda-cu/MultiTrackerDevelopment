...
void RodPenaltyForce::computeForce(const ElasticRod& rod, VecXd& F)
{
    ...
    for (VertexObjectMapIterator voItr=_vertexObjects.begin(); voItr!=_vertexObjects.end(); ++voItr)
    {
        ...
        if (distance < (cmData->getThickness() + rod.radius()))
        {
            ...
            Scalar friction = std::max(rod.getFrictionCoefficient(), cmData->getFrictionCoefficient());
            if (friction > 0.0)
            {
    ...
    for (EdgeRodMapIterator erItr=_edgeRods.begin(); erItr!=_edgeRods.end(); ++erItr)
    {
        ...
        if (distance < (rod.radius() + otherRod->radius()))
        {
            ...
            Scalar e = 1.0;
            if (!(relVel.dot(normal) < 0.0))
                e = rod.getCoefficientOfRestitution();
            ...
            for (int i=0; i<3; ++i)
            {
                F[(3 *   edge1              ) + i] += (1.0 - s1) * force[i];
                F[(3 * ((edge1+1)%_rod.nv())) + i] += (      s1) * force[i];
            }
        }
        else
