...
void RodPenaltyForce::computeForceDX(int baseindex, const ElasticRod& rod, Scalar scale, MatrixBase& J)
{
  ...
  for (int i=0; i<(int) vertex_face_collisions.size(); i++)
  {
    ...
    localJ.setZero();
    localJacobian(localJ, vertex_face_collisions[i].k, vertex_face_collisions[i].n);
...
//    std::cout << "Collision, applying force to vertex " << vertex << ": " << force << std::endl;
      
  }
//  std::cerr << "Forces (AFTER) = \n " << F - beforeF << std::endl;
}

void RodPenaltyForce::addRodPenaltyForce(int vertex, VertexFaceImplicitPenaltyCollision cllsn)
{
//	std::cout << "pntl added " << vertex << " " << cllsn.n << " " << cllsn.cp << "\n";
	vidx.push_back(vertex);
	vertex_face_collisions.push_back(cllsn);
}
