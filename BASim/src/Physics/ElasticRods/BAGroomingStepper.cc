...
//}
//
//BAGroomingStepper::BAGroomingStepper( std::vector<ElasticRod*>& rods,
...
//    for ( int i = 0; i < ( int ) m_edges.size(); ++i )
//        assert(
//                ( m_edges[i].first < m_obj_start && m_edges[i].second < m_obj_start )
//                        || ( m_edges[i].first >= m_obj_start && m_edges[i].second >= m_obj_start ) );
//
//    // For each triangle, ensure that all vertices do indeed belong to a triangulated object
//    for ( int i = 0; i < ( int ) m_faces.size(); ++i )
//        assert(
//                m_faces[i].idx[0] >= m_obj_start && m_faces[i].idx[1] >= m_obj_start
//                        && m_faces[i].idx[2] >= m_obj_start );
//
...
//    i = 0;
//    for ( std::vector<TriangleMesh*>::const_iterator mesh = m_triangle_meshes.begin(); mesh
//            != m_triangle_meshes.end(); mesh++ )
...
//        for ( ElasticRod::vertex_iter itr = m_rods[i]->vertices_begin(); itr
//                != m_rods[i]->vertices_end(); ++itr )
//        {
...
//        for ( TriangleMesh::face_iter fit = m_triangle_meshes[i]->faces_begin(); fit
//                != m_triangle_meshes[i]->faces_end(); ++fit )
//        {
...
//    m_collision_detector = new CollisionDetectorType( m_geodata, m_edges, m_faces, m_dt,
//            m_perf_param.m_skipRodRodCollisions, m_num_threads );
...
//
//    // DEBUG
//    m_total_solver_killed = m_total_collision_killed = m_total_explosion_killed
//            = m_total_stretching_killed = 0;
//
...
//            m_initialLengths[*rod] += ( m_rods[*rod]->getVertex( j ) - m_rods[*rod]->getVertex(
//                    j - 1 ) ).norm();
//}
//
...
//    for ( std::vector<ScriptingController*>::const_iterator scripting_controller =
//            m_scripting_controllers.begin(); scripting_controller != m_scripting_controllers.end(); scripting_controller++ )
...
//
//    TraceStream( g_log, "BAGroomingStepper" ) << "Stepping forward " << selected_steppers.size()
//            << " rod(s).\n";
//
...
//    }
//    DebugStream( g_log, "" ) << "This step solved for "
//            << StaticSolver<GroomingTimeStepper>::solveCounter << " rods\n";
//
...
// */
//void BAGroomingStepper::extractPositions( VecXd& positions, const RodSelectionType& selected_rods,
...
//            positions.segment<3> ( m_base_triangle_indices[i] + 3 * j )
//                    = m_triangle_meshes[i]->getVertex( *vit );
//        }
//    }
//
...
//            velocities.segment<3> ( m_base_dof_indices[*rod] + 3 * j ) = m_rods[*rod]->getVelocity(
//                    j );
//        }
//
//    assert( m_triangle_meshes.size() == m_base_triangle_indices.size() );
...
//}
//
//void BAGroomingStepper::restorePositions( const VecXd& positions,
...
//                m_rods[*rod]->setVertex( j,
//                        positions.segment<3> ( m_base_dof_indices[*rod] + 3 * j ) );
//}
//
//void BAGroomingStepper::restoreVelocities( const VecXd& velocities,
//        const RodSelectionType& selected_rods )
//{
//    assert( m_number_of_rods == m_base_dof_indices.size() );
//
...
//        for ( int j = 0; j < m_rods[*rod]->nv(); ++j )
//            if ( !m_rods[*rod]->getBoundaryCondition()->isVertexScripted( j ) )
//                m_rods[*rod]->setVelocity( j,
//                        velocities.segment<3> ( m_base_dof_indices[*rod] + 3 * j ) );
//}
//
//void BAGroomingStepper::restoreResponses( const VecXd& responses,
//        const RodSelectionType& selected_rods )
//{
//    assert( m_number_of_rods == m_base_dof_indices.size() );
//
...
//        for ( int j = 0; j < m_rods[*rod]->nv(); ++j )
//            if ( !m_rods[*rod]->getBoundaryCondition()->isVertexScripted( j ) )
//                m_rods[*rod]->setResponse( j,
//                        responses.segment<3> ( m_base_dof_indices[*rod] + 3 * j ) );
//}
//
///**
...
//    for ( std::vector<bool>::iterator i = m_collision_immune.begin(); i
//            != m_collision_immune.begin() + m_obj_start; i++ )
//        *i = true;
//    for ( RodSelectionType::const_iterator rod = selected_rods.begin(); rod != selected_rods.end(); rod++ )
...
//                dynamic_cast<EdgeFaceIntersection*> ( collisions.front() );
...
//{
//    TraceStream( g_log, "" ) << "Switching rod-rod collisions " << ( skipRodRodCollisions ? "OFF"
//            : "ON" ) << '\n';
...
//
//    return upper_bound( m_base_vtx_indices.begin(), m_base_vtx_indices.end(), vert_idx )
//            - m_base_vtx_indices.begin() - 1;
//}
//
...
//}
//
//bool BAGroomingStepper::vertexAndFaceShareVertex( const int& v, const int& f0, const int& f1,
//        const int& f2 ) const
//{
...
//
//    return vertexAndFaceShareVertex( vertex, m_faces[face].idx[0], m_faces[face].idx[1],
//            m_faces[face].idx[2] );
//}
//
...
//        if ( !m_lt0_enc )
//            DebugStream( g_log, "" )
//                    << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Encountered scaled collision time "
//                    << time << " less than 0.0. Supressing further messages of this type.\n";
//        m_lt0_enc = true;
//        return false;
//    }
//    if ( time > 1.0 )
...
//            DebugStream( g_log, "" )
//                    << "\033[31;1mWARNING IN BRIDSON STEPPER:\033[m Encountered scaled collision time "
//                    << time << " greater than 1.0. Supressing further messages of this type.\n";
//        m_gt0_enc = true;
//        return false;
//    }
//    return true;
...
//        double radsum = rod.getRadiusScale() * rod.radiusA( i - 1 ) + rod.getRadiusScale()
//                * rod.radiusA( i + 1 );
//        if ( edgelen <= radsum )
//        {
//            DebugStream( g_log, "" )
...
//}
//
//void BAGroomingStepper::computeForces( std::vector<VecXd*> Forces,
//        const RodSelectionType& selected_rods )
//{
//    for ( RodSelectionType::const_iterator rod = selected_rods.begin(); rod != selected_rods.end(); rod++ )
//    {
...
//
//    // Extract masses from the new rod
//    for ( ElasticRod::vertex_iter itr = m_rods.back()->vertices_begin(); itr
//            != m_rods.back()->vertices_end(); ++itr )
//    {
...
//        m_initialLengths.back() += ( m_rods.back()->getVertex( j ) - m_rods.back()->getVertex(
//                j - 1 ) ).norm();
//
//}
//
...
// * Inelastic response
// */
//void BAGroomingStepper::exertVertexImpulse( const Vec3d& I, const double& m, const int& idx,
//        VecXd& v )
//{
//    assert( m > 0.0 );
//    assert( idx >= 0 );
...
//}
//
//void BAGroomingStepper::exertEdgeImpulse( const Vec3d& I, const double& m0, const double& m1,
//        const double& alpha, const int& idx0, const int& idx1, VecXd& v )
//{
//    assert( m0 > 0.0 );
//    assert( m1 > 0.0 );
//    assert( alpha >= 0.0 );
...
//}
//
//void BAGroomingStepper::exertFaceImpulse( const Vec3d& I, const double& m0, const double& m1,
//        const double& m2, const double& u, const double& v, const double& w, const int& idx0,
//        const int& idx1, const int& idx2, VecXd& vel )
//{
//    assert( m0 > 0.0 );
//    assert( m1 > 0.0 );
//    assert( m2 > 0.0 );
...
//
//    exertEdgeImpulse( -I, m_masses[eecol.e0_v0], m_masses[eecol.e0_v1], eecol.s, eecol.e0_v0,
//            eecol.e0_v1, m_vnphalf );
//    exertEdgeImpulse( I, m_masses[eecol.e1_v0], m_masses[eecol.e1_v1], eecol.t, eecol.e1_v0,
//            eecol.e1_v1, m_vnphalf );
//
...
//
//    exertFaceImpulse( -I, m_masses[vfcol.f0], m_masses[vfcol.f1], m_masses[vfcol.f2], vfcol.u,
//            vfcol.v, vfcol.w, vfcol.f0, vfcol.f1, vfcol.f2, m_vnphalf );
...
// */
//bool BAGroomingStepper::executeIterativeInelasticImpulseResponse(
//        std::vector<bool>& failed_collisions_rods, std::vector<bool>& stretching_rods )
//{
...
//    for ( ElasticRod::edge_iter eit = m_rods[rodidx]->edges_begin(); eit
//            != m_rods[rodidx]->edges_end(); ++eit )
//    {
...
//}
//
//void BAGroomingStepper::exertCompliantInelasticVertexFaceImpulse(
//        const VertexFaceCTCollision& vfcol )
//{
...
//        DebugStream( g_log, "" )
//                << "WARNING: attempt to do vertex-face collision with non-dependable rod\n";
//        return;
//    }
//
...
//    const std::vector<int>& scriptedverts =
//            m_rods[rodidx]->getBoundaryCondition()->scriptedVertices();
...
//    {
//        int theidx = 0;
//        for ( ElasticRod::vertex_iter vit = m_rods[rodidx]->vertices_begin(); vit
//                != m_rods[rodidx]->vertices_end(); ++vit )
//        {
...
//        assert(
//                approxEq( m_vnphalf( rodbase + 3 * scriptedverts[i] + 0 ),
//                        desired_values( curdof ), 1.0e-6 ) );
//        ++curdof;
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+1) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rodbase + 3 * scriptedverts[i] + 1 ),
//                        desired_values( curdof ), 1.0e-6 ) );
//        ++curdof;
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+2) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rodbase + 3 * scriptedverts[i] + 2 ),
//                        desired_values( curdof ), 1.0e-6 ) );
//        ++curdof;
//    }
//#endif
...
//}
//
//void BAGroomingStepper::exertCompliantInelasticEdgeEdgeImpulseBothFree(
...
//
//    assert(
//            !YAEdge( eecol.e0_v0, eecol.e0_v1 ).IsFixed( m_geodata ) && !YAEdge( eecol.e1_v0,
//                    eecol.e1_v1 ).IsFixed( m_geodata ) );
//
...
//        DebugStream( g_log, "" )
//                << "WARNING: attempt to do rod-rod collision on non-dependable rods";
//        return;
//    }
//
...
//    const std::vector<int>& scriptedverts0 =
//            m_rods[rod0]->getBoundaryCondition()->scriptedVertices();
...
//#ifndef NDEBUG
//    for ( int i = 0; i < nc0; ++i )
//        for ( ElasticRod::edge_iter eit = m_rods[rod0]->edges_begin(); eit
//                != m_rods[rod0]->edges_end(); ++eit )
...
//    // 'Scripted vertex' constraints
//    const std::vector<int>& scriptedverts1 =
//            m_rods[rod1]->getBoundaryCondition()->scriptedVertices();
...
//#ifndef NDEBUG
//    for ( int i = 0; i < nc1; ++i )
//        for ( ElasticRod::edge_iter eit = m_rods[rod1]->edges_begin(); eit
//                != m_rods[rod1]->edges_end(); ++eit )
...
//        if ( status < 0 )
//            DebugStream( g_log, "" )
//                    << "\033[31;1mWARNING IN IMPLICITEULER:\033[m Problem during linear solve detected. "
//                    << '\n';
//    }
//    // Ensure the edge degrees of freedom experience no impulse
//#ifndef NDEBUG
...
//    {
//        int theidx = 0;
//        for ( ElasticRod::vertex_iter vit = m_rods[rod0]->vertices_begin(); vit
//                != m_rods[rod0]->vertices_end(); ++vit )
//        {
...
//    {
//        int theidx = 0;
//        for ( ElasticRod::vertex_iter vit = m_rods[rod1]->vertices_begin(); vit
//                != m_rods[rod1]->vertices_end(); ++vit )
//        {
...
//    lgrhs( 0 ) = posnn0[0].dot( m_vnphalf.segment( rod0base, rod0nvdof ) ) + posnn1[0].dot(
...
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+0) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rod0base + 3 * scriptedverts0[i] + 0 ), cval0( curdof ),
//                        1.0e-6 ) );
//        ++curdof;
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+1) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rod0base + 3 * scriptedverts0[i] + 1 ), cval0( curdof ),
//                        1.0e-6 ) );
//        ++curdof;
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+2) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rod0base + 3 * scriptedverts0[i] + 2 ), cval0( curdof ),
//                        1.0e-6 ) );
//        ++curdof;
//    }
//#endif
...
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+0) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rod1base + 3 * scriptedverts1[i] + 0 ), cval1( curdof ),
//                        1.0e-6 ) );
//        ++curdof;
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+1) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rod1base + 3 * scriptedverts1[i] + 1 ), cval1( curdof ),
//                        1.0e-6 ) );
//        ++curdof;
//        //TraceStream(g_log, "") << m_vnphalf(rodbase+3*scriptedverts[i]+2) << "   " << desired_values(curdof) << '\n';
//        assert(
//                approxEq( m_vnphalf( rod1base + 3 * scriptedverts1[i] + 2 ), cval1( curdof ),
//                        1.0e-6 ) );
//        ++curdof;
//    }
//#endif
//}
//
//void BAGroomingStepper::exertCompliantInelasticEdgeEdgeImpulseOneFixed(
//        const EdgeEdgeCTCollision& eecol )
//{
...
//    {
//        DebugStream( g_log, "" )
//                << "WARNING: attempt to do edge-edge collision with non-dependable rod\n";
//        return;
//    }
//
...
//}
//
//bool BAGroomingStepper::checkExplosions( std::vector<bool>& exploding_rods,
...
//        {
//            TraceStream( g_log, "" ) << "Rod " << *rod << " Force norms: initial: "
//                    << ( *( m_startForces[*rod] ) ).norm() << " pre-dynamic: "
//                    << ( *( m_preDynamicForces[*rod] ) ).norm() << " pre-collisions: "
//                    << ( *( m_preCollisionForces[*rod] ) ).norm() << " post-collisions: "
//                    << ( *( m_endForces[*rod] ) ).norm() << "\n";
...
//                    TraceStream( g_log, "" ) << "Rod number " << *rod
//                            << " had an explosion during collision response: s = " << s << " p = "
//                            << p << " e = " << e << " rate = " << rate << " \n";
//                }
//            }
//        }
...
//        DebugStream( g_log, "" ) << "Some rods were stretched by a factor > "
//                << m_perf_param.m_stretching_threshold << '\n';
//
...
//    {
//        TraceStream( g_log, "" ) << "Rod number " << rodIdx << " was stretched by a factor "
//                << length / m_initialLengths[rodIdx];
//        return false;
//    }
//
//    return true;
...
// */
//void BAGroomingStepper::setupPenaltyForces( std::list<Collision*>& collisions,
//        const RodSelectionType& selected_rods )
//{
...
//    {
//        return ( m_rod->getVertex( 0 ) - rod1->getVertex( 0 ) ).norm() < ( m_rod->getVertex( 0 )
//                - rod2->getVertex( 0 ) ).norm();
//    }
//};
//
...
//            const std::vector<int>& centerLinePointIndices ) :
...
//{
//    centerLineRods.clear();
//
//    if ( m_wmPeltPoints.size() ) // If we have a set of wmPelt points, find the nearest rod for each of them and declare it to be a center line.
//    {
//        // Prepare a vector of indices (in m_xn) of rod roots
//        std::vector<int> rodRootIndices;
//        for ( RodSelectionType::iterator rod = m_simulated_rods.begin(); rod
//                != m_simulated_rods.end(); rod++ )
//        {
//            rodRootIndices.push_back( m_base_vtx_indices[*rod] );
//            // While we are at it, reset the rod's knowledge of its own centerliness (for display purposes)
//            m_rods[*rod]->setIsClumpCenterLine( false );
//        }
//
//        pantaray::kNN<BVH> knn;
//        BVH bvh;
//        SimplePointBBoxFunctor bboxes( rodRootIndices, m_xn );
//        BVHBuilder<SimplePointBBoxFunctor> ().build( bboxes, &bvh );
//        knn.Setup( bvh );
//
//        // For each wmPelt point, find the nearest among the rod roots
//        const Scalar searchDistance = std::numeric_limits<Scalar>::max();
//        for ( int i = 0; i < m_wmPeltPoints.size(); i += 3 )
//        {
//            Vec3d wmPeltPoint = m_wmPeltPoints.segment<3> ( i );
//            const RodPointDistance<BVH> distance( m_xn, bvh, wmPeltPoint, rodRootIndices );
//            const size_t numberFound = knn.Run( distance, -1.0, searchDistance, 1 );
//            assert( numberFound == 1 );
//
//            const int foundGlobalIdx = rodRootIndices[knn.TopResult().m_node];
//
//            DebugStream( g_log, "" ) << "wmPelt point: " << wmPeltPoint
//                    << "; closest rod root found: " << m_xn.segment<3> ( 3 * foundGlobalIdx )
//                    << '\n';
//
//            const int rodIdx = getContainingRod( foundGlobalIdx );
//            centerLineRods.push_back( rodIdx );
//            m_rods[rodIdx]->setIsClumpCenterLine( true );
//        }
//    }
//    else // Old code left for comparison purposes, to be discarded.
//    {
//        const int rodsPerClump = m_simulated_rods.size() / m_numberOfClumps;
//        int decimationTick = rodsPerClump;
//        for ( RodSelectionType::const_iterator rod = m_simulated_rods.begin(); rod
//                != m_simulated_rods.end(); rod++, decimationTick++ )
//            if ( decimationTick == rodsPerClump )
//            {
//                decimationTick = 0;
...
//
///**
// * Select clumps based on closest rods to a selected few (see findCenterLines)
// */
...
//
//        // In that case, empty each rod's attracting neighbours vector
//        for ( RodSelectionType::const_iterator rod = m_simulated_rods.begin(); rod
//                != m_simulated_rods.end(); rod++ )
...
//    for ( RodSelectionType::const_iterator rod = centerLineRods.begin(); rod
//            != centerLineRods.end(); rod++ )
//        for ( int j = 0; j < m_rods[*rod]->nv(); ++j )
//            centerLinePointIndices.push_back( m_base_vtx_indices[*rod] + j );
//
//    // Build a bvh (this reorders centerLinePointIndices) and the kNN finder
//    pantaray::kNN<BVH> knn;
//    BVH bvh;
...
//            assert(
//                    find( centerLineRods.begin(), centerLineRods.end(), foundRodIdx )
//                            != centerLineRods.end() ); // found rod should be in the center lines list
//
...
// const size_t numberFound = knn.Run(distance, -1.0f, searchDistance, m_numberOfClumps);
