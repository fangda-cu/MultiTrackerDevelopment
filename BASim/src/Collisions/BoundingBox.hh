...
template<typename T>
bool Intersect(BoundingBox<T> bbox_a, BoundingBox<T> bbox_b)
{
	// Attempt to find a separating hyperplane
	if (bbox_a.max[0] < bbox_b.min[0])
		return false;
	if (bbox_b.max[0] < bbox_a.min[0])
		return false;
	if (bbox_a.max[1] < bbox_b.min[1])
		return false;
	if (bbox_b.max[1] < bbox_a.min[1])
		return false;
	if (bbox_a.max[2] < bbox_b.min[2])
		return false;
	if (bbox_b.max[2] < bbox_a.min[2])
		return false;
