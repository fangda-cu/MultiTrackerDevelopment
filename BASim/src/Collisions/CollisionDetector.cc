...
CollisionDetector::CollisionDetector(const VecXd& x, const VecXd& v, const std::vector<std::pair<int, int> >& edges,
	...
	m_geodata(x, v, vertex_radii, masses, obj_start), m_time_step(timestep), m_cllsns(NULL)
{
	m_objects.reserve(edges.size() + faces.size());

	for (uint32_t i = 0; i < edges.size(); i++)
		m_objects.push_back(new YAEdge(edges[i]));
	for (uint32_t i = 0; i < faces.size(); i++)
		m_objects.push_back(new YATriangle(faces[i]));

	GeometryBBoxFunctor bboxfunctor(m_objects, m_geodata);
	MiddleBVHBuilder bvh_builder;
	bvh_builder.build(bboxfunctor, &m_bvh);

	std::cerr << "Number of edges = " << edges.size() << std::endl;
	std::cerr << "Number of triangles = " << faces.size() << std::endl;
	std::cerr << "Number of objects = " << m_objects.size() << std::endl;
	std::cerr << "Number of points = " << x.size() / 3 << std::endl;
	std::cerr << "Number of velocities = " << x.size() / 3 << std::endl;
	std::cerr << "Number of radii = " << vertex_radii.size() << std::endl;
	std::cerr << "Number of masses = " << masses.size() << std::endl;
...
void CollisionDetector::getContinuousTimeCollisions(std::list<ContinuousTimeCollision>& cllsns)
{
	...
	expandBoundingBoxes(root);
	computeContinuousTimeCollisions(root, root);
}

void CollisionDetector::updateContinuousTimeCollisions()
{
	for (std::list<ContinuousTimeCollision>::iterator i = m_cllsns->begin(); i != m_cllsns->end(); i++)
	{
...
void CollisionDetector::expandBoundingBoxes(BVHNode& node)
{
	if (node.IsLeaf()) // Expand the leaf's bounding box to contain all its objects
	{
		...
		{
			TopologicalElement* object = m_objects[i];
			bbox.Insert(object->GetBBox(m_geodata, m_time_step));
		}
	}
	else // Honey, I expanded the kids...
	{
		BVHNode hansel = m_bvh.GetNode(node.ChildIndex());
		expandBoundingBoxes(hansel);
		node.BBox().Insert(hansel.BBox());
		BVHNode gretel = m_bvh.GetNode(node.ChildIndex() + 1);
		expandBoundingBoxes(gretel);
		node.BBox().Insert(gretel.BBox());
	}
}
...
void CollisionDetector::intersectContent(const BVHNode& node_a, const BVHNode& node_b)
{
	...
	for (uint32_t i = leaf_a_begin; i < leaf_a_end; ++i)
		...
		{
			TopologicalElement* object_a = m_objects[i];
			TopologicalElement* object_b = m_objects[j];
...
void CollisionDetector::appendContinuousTimeIntersection(const YAEdge* edge_a, const YAEdge* edge_b)
{
	...
	edgeXedge.e0_v0 = edge_a->m_edge.first;
	edgeXedge.e0_v1 = edge_a->m_edge.second;
	edgeXedge.e1_v0 = edge_b->m_edge.first;
	edgeXedge.e1_v1 = edge_b->m_edge.second;
	...
	if (analyseCollision(edgeXedge))
		m_cllsns->push_back(ContinuousTimeCollision(edgeXedge));
}
...
void CollisionDetector::appendContinuousTimeIntersection(int v_index, const YATriangle* triangle)
{
	...
	vertexXface.f0 = triangle->m_triangle.idx[0];
	vertexXface.f1 = triangle->m_triangle.idx[1];
	vertexXface.f2 = triangle->m_triangle.idx[2];
	...
	if (analyseCollision(vertexXface))
		m_cllsns->push_back(ContinuousTimeCollision(vertexXface));
}

void CollisionDetector::appendContinuousTimeIntersection(const YAEdge* edge, const YATriangle* triangle)
{
	YAEdge edge_2(triangle->m_triangle.idx[0], triangle->m_triangle.idx[1]);
	YAEdge edge_1(triangle->m_triangle.idx[2], triangle->m_triangle.idx[0]);
	YAEdge edge_0(triangle->m_triangle.idx[1], triangle->m_triangle.idx[2]);
	...
	appendContinuousTimeIntersection(edge->m_edge.first, triangle);
	appendContinuousTimeIntersection(edge->m_edge.second, triangle);
}
