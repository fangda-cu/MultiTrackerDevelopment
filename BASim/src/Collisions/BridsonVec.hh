...
struct Vec
{
   ...
   Vec<N,T> operator/(const Vec<N,T> &w) const
   {
      Vec<N,T> componentwise_quotient;
      for(unsigned int i=0; i<N; ++i) componentwise_quotient[i]=v[i]/w.v[i];
      return componentwise_quotient;
   }
};
...
{ 
   Vec<N,int> rounded;
   for(unsigned int i=0; i<N; ++i)
      rounded.v[i]=(int)std::ceil(a.v[i]);
   return rounded; 
}
