...
class TetrahedronPair
{
    typedef double point[3];
    point *V1, *V2; // vertices coordinates
    double e_v1[6][3], e_v2[6][3]; // vectors edge-oriented
    int masks[4]; // for each face of the first tetrahedron, stores the halfspace each vertex of the second tetrahedron belongs to
    double P_V1[4][3], P_V2[4][3]; // differences between the vertices of the second (first) tetrahedron and the vertex 0  of the first(second) tetrahedron
    double Coord_1[4][4], Coord_2[4][4]; // vertices coordinates in the affine space
    double n[3]; // variable to store the normals

public:
    TetrahedronPair(double V_1[4][3], double V_2[4][3]) :
        V1(V_1), V2(V_2)
    {
    }
...
private:
    ...
    // it is the same as FaceA_1, only the values V2[0]-v_ref are used only for the fourth face hence they do not need to be stored
    ...
    bool FaceB_1()
    {
        ...
        return ((DOT(P_V2[0] , n) > 0) && (DOT(P_V2[1] , n) > 0) && (DOT(P_V2[2] , n) > 0) && (DOT(P_V2[3] , n) > 0));
    }

    bool FaceB_2()
    {
        double * v_ref = V2[1];
        return ((SUB_DOT(V1[0],v_ref , n ) > 0) && (SUB_DOT(V1[1],v_ref , n ) > 0) && (SUB_DOT(V1[2],v_ref , n ) > 0)
                && (SUB_DOT(V1[3],v_ref , n ) > 0));
    }

    // EdgeA -------------------------------------------------------
    bool EdgeA(const int & f0, const int & f1)
    {
...
inline bool TetrahedronPair::DoOverlap()
{
