...
public:
	explicit ContinuousTimeCollision(const EdgeEdgeContinuousTimeCollision& edgedg) :
		m_edgedg(true), m_edgedgcllsn(edgedg), m_vrtfcecllsn()
	{
	}

	explicit ContinuousTimeCollision(const VertexFaceContinuousTimeCollision& vrtfce) :
		m_edgedg(false), m_edgedgcllsn(), m_vrtfcecllsn(vrtfce)
	{
	}
...
struct EdgeVertexEdgePair
{
	...
	bool operator<(const EdgeVertexEdgePair& rhs) const
	{
		...
		if (this->freeedge == rhs.freeedge)
		{
			...
			if (this->facevert0 == rhs.facevert0)
			{
				if (this->facevert1 < rhs.facevert1)
					return true;
			}
		}
		return false;
...
{
public:
	BVHAABB(const VecXd& x, const VecXd& v, const std::vector<std::pair<int, int> >& edges,
			const std::vector<TriangularFace>& faces, const std::vector<double>& vert_radii, const double& timestep) :
		m_root_node(NULL), m_x(x), m_v(v), m_edges(edges), m_faces(faces), m_vert_radii(vert_radii), m_h(timestep)
	{
	...
	void regenerateBVH()
	{
		...
		for (int i = 0; i < (int) m_edges.size(); ++i)
			edge_indcs.push_back(i);
		std::vector<int> face_indcs;
		for (int i = 0; i < (int) m_faces.size(); ++i)
			face_indcs.push_back(i);
	...
	void getProximityCollisions(std::vector<EdgeEdgeProximityCollision>& edg_edg,
			std::vector<VertexFaceProximityCollision>& vrt_fce)
	{
		if (m_x.size() == 0)
			return;
		...
		for (std::set<std::pair<int, int> >::const_iterator it = vrt_fce_set.begin(); it != vrt_fce_set.end(); ++it)
		{
			...
			assert(it->first >= 0);
			assert(it->first < (int) m_vert_radii.size());
			vrt_fce_prxm.v0 = it->first;

			assert(it->second >= 0);
			assert(it->second < (int) m_faces.size());

			assert(m_faces[it->second].idx[0] >= 0);
			assert(m_faces[it->second].idx[0] < (int) m_vert_radii.size());
			vrt_fce_prxm.f0 = m_faces[it->second].idx[0];
			assert(m_faces[it->second].idx[1] >= 0);
			assert(m_faces[it->second].idx[1] < (int) m_vert_radii.size());
			vrt_fce_prxm.f1 = m_faces[it->second].idx[1];
			assert(m_faces[it->second].idx[2] >= 0);
			assert(m_faces[it->second].idx[2] < (int) m_vert_radii.size());
	...
	void getContinuousTimeCollisions(std::list<ContinuousTimeCollision>& cllsns)
	{
		...
		for (int i = 0; i < (int) edgedg.size(); ++i)
			cllsns.push_back(ContinuousTimeCollision(edgedg[i]));
		// edgedg.clear();
		for (int i = 0; i < (int) vrtfce.size(); ++i)
			cllsns.push_back(ContinuousTimeCollision(vrtfce[i]));
		// vrtfce.clear();
	}

	void getContinuousTimeCollisions(std::vector<EdgeEdgeContinuousTimeCollision>& edg_edg,
			std::vector<VertexFaceContinuousTimeCollision>& vrt_fce)
	{
		if (m_x.size() == 0)
			return;
	...
	void getImplicitPenaltyCollisions(std::vector<EdgeEdgeProximityCollision>& edg_edg,
			std::vector<VertexFaceProximityCollision>& vrt_fce)
	{
		if (m_x.size() == 0)
			return;
...
private:
	...
	void printTreeInfo(BVHAABBNode* node, int depth, const std::string left_or_right)
	{
		...
		if (node->children[0] == NULL && node->children[1] == NULL)
		{
			...
			int numedges = 0;
			int numfaces = 0;
			computeNumEnclosedObjects(node, numedges, numfaces);
			std::cout << "     Num objects: " << numedges << " " << numfaces << std::endl;
			std::cout << "     Edges: ";
			for (int i = 0; i < (int) node->edge_indcs.size(); ++i)
				std::cout << node->edge_indcs[i] << " ";
			std::cout << std::endl;
			std::cout << "     Faces: ";
			for (int i = 0; i < (int) node->face_indcs.size(); ++i)
				std::cout << node->face_indcs[i] << " ";
			std::cout << std::endl;
	...
	void getContainedObjects(BVHAABBNode* node, std::set<int>& edges, std::set<int>& faces)
	{
		...
		{
			for (int i = 0; i < (int) node->edge_indcs.size(); ++i)
				edges.insert(node->edge_indcs[i]);
			for (int i = 0; i < (int) node->face_indcs.size(); ++i)
				faces.insert(node->face_indcs[i]);
		}
	...
	int computeDepth(BVHAABBNode* node)
	{
		...
		if (isLeaf(node))
			...
			return computeDepth(node->children[0]) + 1;
		else if (!hasLeftChild(node) && hasRightChild(node))
			return computeDepth(node->children[1]) + 1;
		else
			return std::max(computeDepth(node->children[0]), computeDepth(node->children[1])) + 1;
	}
	...
	bool hasProperChildren(BVHAABBNode* node)
	{
		...
		if ((!hasLeftChild(node) && hasRightChild(node)) || (hasLeftChild(node) && !hasRightChild(node)))
			return false;
	...
	void computeNumEnclosedObjects(BVHAABBNode* node, int& num_edges, int& num_faces)
	{
		...
		if (hasLeftChild(node))
			computeNumEnclosedObjects(node->children[0], num_edges, num_faces);
		if (hasRightChild(node))
			computeNumEnclosedObjects(node->children[1], num_edges, num_faces);
	}
	...
	bool parentsContainChildren(BVHAABBNode* node)
	{
		...
		if (hasLeftChild(node))
		{
			if (node->lwr_bnds[0] > node->children[0]->lwr_bnds[0])
				return false;
			if (node->lwr_bnds[1] > node->children[0]->lwr_bnds[1])
				return false;
			if (node->lwr_bnds[2] > node->children[0]->lwr_bnds[2])
				return false;

			if (node->upr_bnds[0] < node->children[0]->upr_bnds[0])
				return false;
			if (node->upr_bnds[1] < node->children[0]->upr_bnds[1])
				return false;
			if (node->upr_bnds[2] < node->children[0]->upr_bnds[2])
				return false;

			if (!parentsContainChildren(node->children[0]))
				return false;
		}
		// Check the right child
		if (hasRightChild(node))
		{
			if (node->lwr_bnds[0] > node->children[1]->lwr_bnds[0])
				return false;
			if (node->lwr_bnds[1] > node->children[1]->lwr_bnds[1])
				return false;
			if (node->lwr_bnds[2] > node->children[1]->lwr_bnds[2])
				return false;

			if (node->upr_bnds[0] < node->children[1]->upr_bnds[0])
				return false;
			if (node->upr_bnds[1] < node->children[1]->upr_bnds[1])
				return false;
			if (node->upr_bnds[2] < node->children[1]->upr_bnds[2])
				return false;

			if (!parentsContainChildren(node->children[1]))
				return false;
		}
	...
	bool childrenPointersSetupCorrectly(BVHAABBNode* node)
	{
		...
		if (hasLeftChild(node))
			if (!childrenPointersSetupCorrectly(node->children[0]))
				return false;

		if (hasRightChild(node))
			if (!childrenPointersSetupCorrectly(node->children[1]))
				return false;
	...
	bool treeBuiltCorrectly()
	{
		...
		for (std::set<int>::const_iterator i = edges.begin(); i != edges.end(); ++i)
			if (*i >= (int) m_edges.size())
				all_object_valid = false;
		for (std::set<int>::const_iterator i = faces.begin(); i != faces.end(); ++i)
			if (*i >= (int) m_faces.size())
				all_object_valid = false;

		// Ensure the children are contained in the parents
		bool children_in_parents = parentsContainChildren(m_root_node);

		return children_different && proper_children && leaves_have_objects && proper_num_enclosed && all_objects_enclosed
				&& all_object_valid && children_in_parents;
	}

	/////////////////////////////////////////////////////////////////////////////
	// Methods (M-E-T-H-O-D MAN, M-E-T-H-O-D MAN)

	bool isLeaf(BVHAABBNode* node) const
	{
		assert(node != NULL);
		return node->children[0] == NULL && node->children[1] == NULL;
	}

	bool hasLeftChild(BVHAABBNode* node) const
	{
		assert(node != NULL);
		return node->children[0] != NULL;
	}

	bool hasRightChild(BVHAABBNode* node) const
	{
		assert(node != NULL);
		return node->children[1] != NULL;
	}

	int numContainedObjects(BVHAABBNode* node) const
	{
		assert(node != NULL);
		return int(node->edge_indcs.size() + node->face_indcs.size());
	}

	double computeNodeVolume(BVHAABBNode* node) const
	{
		assert(node != NULL);
		return (node->upr_bnds[0] - node->lwr_bnds[0]) * (node->upr_bnds[1] - node->lwr_bnds[1]) * (node->upr_bnds[2]
				- node->lwr_bnds[2]);
	}

	void computePossibleProximityCollisions(std::set<IntPair>& edg_edg, std::set<std::pair<int, int> >& vrt_fce,
			std::set<std::pair<int, int> >& edg_fce, std::set<IntPair>& fce_fce, BVHAABBNode* node_a, BVHAABBNode* node_b) const
	{
		assert(node_a != NULL);
		assert(node_b != NULL);
		// If the bounding volumes do not overlap, there are no possible collisions between their objects
		if (!AABBsOverlap(node_a, node_b))
			return;
		// If both bounding volumes are leaves, add their contents to list potential collisions
		if (isLeaf(node_a) && isLeaf(node_b))
			compareContentsProximity(edg_edg, vrt_fce, edg_fce, fce_fce, node_a, node_b);
	...
	bool AABBsOverlap(BVHAABBNode* node_a, BVHAABBNode* node_b) const
	{
		assert(node_a != NULL);
		assert(node_b != NULL);

		// Attempt to find a separating hyperplane
		if (node_a->upr_bnds[0] < node_b->lwr_bnds[0])
			return false;
		if (node_b->upr_bnds[0] < node_a->lwr_bnds[0])
			return false;
		if (node_a->upr_bnds[1] < node_b->lwr_bnds[1])
			return false;
		if (node_b->upr_bnds[1] < node_a->lwr_bnds[1])
			return false;
		if (node_a->upr_bnds[2] < node_b->lwr_bnds[2])
			return false;
		if (node_b->upr_bnds[2] < node_a->lwr_bnds[2])
			return false;
	...
	void compareContentsProximity(std::set<IntPair>& edg_edg, std::set<std::pair<int, int> >& vrt_fce,
			std::set<std::pair<int, int> >& edg_fce, std::set<IntPair>& fce_fce, BVHAABBNode* node_a, BVHAABBNode* node_b) const
	{
		assert(node_a != NULL);
		assert(node_b != NULL);
		assert(isLeaf(node_a));
		assert(isLeaf(node_b));
		assert(numContainedObjects(node_a) > 0);
		assert(numContainedObjects(node_b) > 0);
	...
	void compareEdgeLists(std::vector<int>& edges_a, std::vector<int>& edges_b, std::set<IntPair>& edg_edg) const
	{
		for (int i = 0; i < (int) edges_a.size(); ++i)
			for (int j = 0; j < (int) edges_b.size(); ++j)
			{
				assert(edges_a[i] >= 0);
				assert(edges_a[i] < (int) m_edges.size());
				assert(m_edges[edges_a[i]].first < (int) m_vert_radii.size());
				assert(m_edges[edges_a[i]].second < (int) m_vert_radii.size());
				assert(edges_b[j] >= 0);
				assert(edges_b[j] < (int) m_edges.size());
				...
				if (edges_a[i] == edges_b[j])
					continue;
				if (m_edges[edges_a[i]].second == m_edges[edges_b[j]].first || m_edges[edges_a[i]].first
						== m_edges[edges_b[j]].second)
					continue;
	...
	void compareEdgeFaceLists(std::vector<int>& edges, std::vector<int>& faces, std::set<IntPair>& edg_edg,
			std::set<std::pair<int, int> >& vrt_fce, std::set<std::pair<int, int> >& edg_fce) const
	{
		for (int i = 0; i < (int) edges.size(); ++i)
			for (int j = 0; j < (int) faces.size(); ++j)
			{
				assert(edges[i] >= 0);
				assert(edges[i] < (int) m_edges.size());
				assert(m_edges[edges[i]].first >= 0);
				assert(m_edges[edges[i]].first < (int) m_vert_radii.size());
				assert(m_edges[edges[i]].second >= 0);
				assert(m_edges[edges[i]].second < (int) m_vert_radii.size());
				assert(faces[j] >= 0);
				assert(faces[j] < (int) m_faces.size());
				assert(m_faces[faces[j]].idx[0] >= 0);
				assert(m_faces[faces[j]].idx[0] < (int) m_vert_radii.size());
				assert(m_faces[faces[j]].idx[1] >= 0);
				assert(m_faces[faces[j]].idx[1] < (int) m_vert_radii.size());
				assert(m_faces[faces[j]].idx[2] >= 0);
				assert(m_faces[faces[j]].idx[2] < (int) m_vert_radii.size());
	...
	void compareFaceLists(std::vector<int>& faces_a, std::vector<int>& faces_b, std::set<IntPair>& edg_edg,
			std::set<std::pair<int, int> >& vrt_fce, std::set<IntPair>& fce_fce) const
	{
	...
	void recomputeProximityBoundsBottomUp(BVHAABBNode* node)
	{
		...
		if (isLeaf(node))
			computeProximityBounds(node->edge_indcs, node->face_indcs, node);
	...
	void computeProximityBounds(const std::vector<int>& edge_indcs, const std::vector<int>& face_indcs, BVHAABBNode* box)
	{
		...
		{
			assert(edge_indcs[i] >= 0);
			assert(edge_indcs[i] < (int) m_edges.size());
			assert(m_edges[edge_indcs[i]].first >= 0);
			assert(m_edges[edge_indcs[i]].first < (int) m_vert_radii.size());
			assert(m_edges[edge_indcs[i]].second >= 0);
			assert(m_edges[edge_indcs[i]].second < (int) m_vert_radii.size());

			const double r0 = m_vert_radii[m_edges[edge_indcs[i]].first];
			assert(r0 >= 0.0);
			const double r1 = m_vert_radii[m_edges[edge_indcs[i]].second];
			assert(r1 >= 0.0);
		...
		{
			assert(face_indcs[i] >= 0);
			assert(face_indcs[i] < (int) m_faces.size());
			assert(m_faces[face_indcs[i]].idx[0] >= 0);
			assert(m_faces[face_indcs[i]].idx[0] < (int) m_vert_radii.size());
			assert(m_faces[face_indcs[i]].idx[1] >= 0);
			assert(m_faces[face_indcs[i]].idx[1] < (int) m_vert_radii.size());
			assert(m_faces[face_indcs[i]].idx[2] >= 0);
			assert(m_faces[face_indcs[i]].idx[2] < (int) m_vert_radii.size());

			const double r0 = m_vert_radii[m_faces[face_indcs[i]].idx[0]];
			assert(r0 >= 0.0);
			const double r1 = m_vert_radii[m_faces[face_indcs[i]].idx[1]];
			assert(r1 >= 0.0);
			const double r2 = m_vert_radii[m_faces[face_indcs[i]].idx[2]];
			assert(r2 >= 0.0);
	...
	double computeEdgeCentroidX(const int& edg_idx) const
	{
		assert(edg_idx >= 0);
		assert(edg_idx < (int) m_edges.size());
		assert(m_edges[edg_idx].first >= 0);
		assert(3 * m_edges[edg_idx].first < m_x.size());
		assert(m_edges[edg_idx].second >= 0);
		assert(3 * m_edges[edg_idx].second < m_x.size());

		return 0.5 * (m_x(3 * m_edges[edg_idx].first) + m_x(3 * m_edges[edg_idx].second));
	}

	double computeFaceCentroidX(const int& fce_idx) const
	{
		assert(fce_idx >= 0);
		assert(fce_idx < (int) m_faces.size());
		assert(m_faces[fce_idx].idx[0] >= 0);
		assert(3 * m_faces[fce_idx].idx[0] < m_x.size());
		assert(m_faces[fce_idx].idx[1] >= 0);
		assert(3 * m_faces[fce_idx].idx[1] < m_x.size());
		assert(m_faces[fce_idx].idx[2] >= 0);
		assert(3 * m_faces[fce_idx].idx[2] < m_x.size());
	...
	bool splitOnXMedian(const std::vector<int>& edge_indcs, const std::vector<int>& face_indcs, BVHAABBNode* to_divide)
	{
		...
		for (int i = 0; i < (int) edge_indcs.size(); ++i)
			xmedians.push_back(computeEdgeCentroidX(edge_indcs[i]));
		for (int i = 0; i < (int) face_indcs.size(); ++i)
			xmedians.push_back(computeFaceCentroidX(face_indcs[i]));
		std::sort(xmedians.begin(), xmedians.end());
		double xmedian;
		if (xmedians.size() % 2 == 1)
			xmedian = xmedians[(xmedians.size() - 1) / 2];
		else
			xmedian = 0.5 * (xmedians[xmedians.size() / 2] + xmedians[(xmedians.size() / 2) - 1]);
		...
		for (int i = 0; i < (int) xlftedgs.size(); ++i)
			testedges.insert(xlftedgs[i]);
		for (int i = 0; i < (int) xrhtedgs.size(); ++i)
			testedges.insert(xrhtedgs[i]);
		assert(testedges.size() == edge_indcs.size());

		std::set<int> testfaces;
		for (int i = 0; i < (int) xlftfaces.size(); ++i)
			testfaces.insert(xlftfaces[i]);
		for (int i = 0; i < (int) xrhtfaces.size(); ++i)
			testfaces.insert(xrhtfaces[i]);
		assert(testfaces.size() == face_indcs.size());
		//#endif

		// If we were not able to split about the median
		if ((xlftedgs.size() + xlftfaces.size()) == 0 || (xrhtedgs.size() + xrhtfaces.size()) == 0)
			return false;
	...
	double computeEdgeCentroidY(const int& edg_idx) const
	{
		assert(edg_idx >= 0);
		assert(edg_idx < (int) m_edges.size());
		assert(m_edges[edg_idx].first >= 0);
		assert(3 * m_edges[edg_idx].first + 1 < m_x.size());
		assert(m_edges[edg_idx].second >= 0);
		assert(3 * m_edges[edg_idx].second + 1 < m_x.size());

		return 0.5 * (m_x(3 * m_edges[edg_idx].first + 1) + m_x(3 * m_edges[edg_idx].second + 1));
	}

	double computeFaceCentroidY(const int& fce_idx) const
	{
		assert(fce_idx >= 0);
		assert(fce_idx < (int) m_faces.size());
		assert(m_faces[fce_idx].idx[0] >= 0);
		assert(3 * m_faces[fce_idx].idx[0] + 1 < m_x.size());
		assert(m_faces[fce_idx].idx[1] >= 0);
		assert(3 * m_faces[fce_idx].idx[1] + 1 < m_x.size());
		assert(m_faces[fce_idx].idx[2] >= 0);
		assert(3 * m_faces[fce_idx].idx[2] + 1 < m_x.size());

		return (m_x(3 * m_faces[fce_idx].idx[0] + 1) + m_x(3 * m_faces[fce_idx].idx[1] + 1) + m_x(
				3 * m_faces[fce_idx].idx[2] + 1)) / 3.0;
	}
	...
	bool splitOnYMedian(const std::vector<int>& edge_indcs, const std::vector<int>& face_indcs, BVHAABBNode* to_divide)
	{
		...
		for (int i = 0; i < (int) edge_indcs.size(); ++i)
			ymedians.push_back(computeEdgeCentroidY(edge_indcs[i]));
		for (int i = 0; i < (int) face_indcs.size(); ++i)
			ymedians.push_back(computeFaceCentroidY(face_indcs[i]));
		std::sort(ymedians.begin(), ymedians.end());
		double ymedian;
		if (ymedians.size() % 2 == 1)
			ymedian = ymedians[(ymedians.size() - 1) / 2];
		else
			ymedian = 0.5 * (ymedians[ymedians.size() / 2] + ymedians[(ymedians.size() / 2) - 1]);
		...
		//#ifdef DEBUG
		std::set<int> testedges;
		for (int i = 0; i < (int) ylftedgs.size(); ++i)
			testedges.insert(ylftedgs[i]);
		for (int i = 0; i < (int) yrhtedgs.size(); ++i)
			testedges.insert(yrhtedgs[i]);
		assert(testedges.size() == edge_indcs.size());

		std::set<int> testfaces;
		for (int i = 0; i < (int) ylftfaces.size(); ++i)
			testfaces.insert(ylftfaces[i]);
		for (int i = 0; i < (int) yrhtfaces.size(); ++i)
			testfaces.insert(yrhtfaces[i]);
		assert(testfaces.size() == face_indcs.size());
		//#endif

		// If we were not able to split about the median
		if ((ylftedgs.size() + ylftfaces.size()) == 0 || (yrhtedgs.size() + yrhtfaces.size()) == 0)
			return false;
	...
	double computeEdgeCentroidZ(const int& edg_idx) const
	{
		assert(edg_idx >= 0);
		assert(edg_idx < (int) m_edges.size());
		assert(m_edges[edg_idx].first >= 0);
		assert(3 * m_edges[edg_idx].first + 2 < m_x.size());
		assert(m_edges[edg_idx].second >= 0);
		assert(3 * m_edges[edg_idx].second + 2 < m_x.size());

		return 0.5 * (m_x(3 * m_edges[edg_idx].first + 2) + m_x(3 * m_edges[edg_idx].second + 2));
	}

	double computeFaceCentroidZ(const int& fce_idx) const
	{
		assert(fce_idx >= 0);
		assert(fce_idx < (int) m_faces.size());
		assert(m_faces[fce_idx].idx[0] >= 0);
		assert(3 * m_faces[fce_idx].idx[0] + 2 < m_x.size());
		assert(m_faces[fce_idx].idx[1] >= 0);
		assert(3 * m_faces[fce_idx].idx[1] + 2 < m_x.size());
		assert(m_faces[fce_idx].idx[2] >= 0);
		assert(3 * m_faces[fce_idx].idx[2] + 2 < m_x.size());

		return (m_x(3 * m_faces[fce_idx].idx[0] + 2) + m_x(3 * m_faces[fce_idx].idx[1] + 2) + m_x(
				3 * m_faces[fce_idx].idx[2] + 2)) / 3.0;
	}
	...
	bool splitOnZMedian(const std::vector<int>& edge_indcs, const std::vector<int>& face_indcs, BVHAABBNode* to_divide)
	{
		...
		for (int i = 0; i < (int) edge_indcs.size(); ++i)
			zmedians.push_back(computeEdgeCentroidZ(edge_indcs[i]));
		for (int i = 0; i < (int) face_indcs.size(); ++i)
			zmedians.push_back(computeFaceCentroidZ(face_indcs[i]));
		std::sort(zmedians.begin(), zmedians.end());
		double zmedian;
		if (zmedians.size() % 2 == 1)
			zmedian = zmedians[(zmedians.size() - 1) / 2];
		else
			zmedian = 0.5 * (zmedians[zmedians.size() / 2] + zmedians[(zmedians.size() / 2) - 1]);
		...
		//#ifdef DEBUG
		std::set<int> testedges;
		for (int i = 0; i < (int) zlftedgs.size(); ++i)
			testedges.insert(zlftedgs[i]);
		for (int i = 0; i < (int) zrhtedgs.size(); ++i)
			testedges.insert(zrhtedgs[i]);
		assert(testedges.size() == edge_indcs.size());

		std::set<int> testfaces;
		for (int i = 0; i < (int) zlftfaces.size(); ++i)
			testfaces.insert(zlftfaces[i]);
		for (int i = 0; i < (int) zrhtfaces.size(); ++i)
			testfaces.insert(zrhtfaces[i]);
		assert(testfaces.size() == face_indcs.size());
		//#endif

		// If we were not able to split about the median
		if ((zlftedgs.size() + zlftfaces.size()) == 0 || (zrhtedgs.size() + zrhtfaces.size()) == 0)
			return false;
	...
	void divideNode(const std::vector<int>& edge_indcs, const std::vector<int>& face_indcs, BVHAABBNode* to_divide)
	{
		...
		{
			if (axis_extents[i].second == 0)
			{
				if (splitOnXMedian(edge_indcs, face_indcs, to_divide))
				{
					return;
				}
			}
			else if (axis_extents[i].second == 1)
			{
				if (splitOnYMedian(edge_indcs, face_indcs, to_divide))
				{
					return;
				}
			}
			else if (axis_extents[i].second == 2)
			{
				if (splitOnZMedian(edge_indcs, face_indcs, to_divide))
				{
					return;
				}
			}
	...
	void recomputeContinuousTimeBoundsBottomUp(BVHAABBNode* node)
	{
		...
		// If node is a leaf, update the bounding box for that node
		if (isLeaf(node))
			computeContinuousBounds(node->edge_indcs, node->face_indcs, node);
		// Otherwise update the node's children bounding box's and compute based off them
		else
		{
			assert(hasLeftChild(node));
	...
	void computeContinuousBounds(const std::vector<int>& edge_indcs, const std::vector<int>& face_indcs, BVHAABBNode* box)
	{
		...
		for (int i = 0; i < (int) edge_indcs.size(); ++i)
		{
			...
			const double r1 = m_vert_radii[m_edges[edge_indcs[i]].second];
			assert(r1 >= 0.0);
			const Vec3d& x0 = m_x.segment<3> (3 * m_edges[edge_indcs[i]].first) + m_h * m_v.segment<3> (
					3 * m_edges[edge_indcs[i]].first);
			const Vec3d& x1 = m_x.segment<3> (3 * m_edges[edge_indcs[i]].second) + m_h * m_v.segment<3> (
					3 * m_edges[edge_indcs[i]].second);
		...
		for (int i = 0; i < (int) face_indcs.size(); ++i)
		{
			...
			const double r2 = m_vert_radii[m_faces[face_indcs[i]].idx[2]];
			assert(r2 >= 0.0);
			const Vec3d& x0 = m_x.segment<3> (3 * m_faces[face_indcs[i]].idx[0]) + m_h * m_v.segment<3> (
					3 * m_faces[face_indcs[i]].idx[0]);
			const Vec3d& x1 = m_x.segment<3> (3 * m_faces[face_indcs[i]].idx[1]) + m_h * m_v.segment<3> (
					3 * m_faces[face_indcs[i]].idx[1]);
			const Vec3d& x2 = m_x.segment<3> (3 * m_faces[face_indcs[i]].idx[2]) + m_h * m_v.segment<3> (
					3 * m_faces[face_indcs[i]].idx[2]);
