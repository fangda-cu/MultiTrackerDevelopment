...
void YImage::resize(int width, int height)
{
  // do nothing
  ...
  YPixel* new_data = (YPixel*) malloc(width * height * sizeof(YPixel));
  memset(new_data, 0, width * height * sizeof(YPixel));

  if (m_data)
    {
      int min_width = std::min(m_width, width);
      int min_height = std::min(m_height, height);

      for(int j = 0; j < min_height; ++j)
        memcpy(new_data + j*width, m_data + j*m_width, min_width * sizeof(YPixel));
...
bool YImage::save(const char* fname) const
{
  ...
  // We'll use the low-level interface since the high-level interface
  // won't handle png_set_filler() which we need to tell libpng to
  // strip out the A from our 4-byte pixels.
  ...
  */
  ...
  // Finally we create a row_pointers[] pointing into our data* and
  // write the png out to the FILE*.
  {
...
bool YImage::load(const char* fname)
{
  ...
  // We'll use the low-level interface since the high-level interface
  // won't handle png_set_filler() which we need to guarantee there'll
  // be a filler "A" in our 4-byte ARGB pixels.  Really the low-level
  // interface isn't more complicated than the high-level interface.
  // To choose transform flags we have to query the png_info struct.
  // Instead of OR'ing in another transform flag (high-level
  // interface), we call a set method (low-level interface).

  // First we read the info struct.
  png_read_info(png_ptr, info_ptr);

  // Now we set up transforms.
  // 1. convert gray and paletted to rgb (this guarantees 8 or 16 bit
  //    depths (rgb must be 8 or 16)).  also expand the alpha color to
  //    an alpha channel and convert 16 bit depths to 8.
  // 2. if we don't have alpha, add an opaque channel.  also swap RGB
  //    and BGR depending on YPixel.
 ...
 YImage_load_cleanup:
  // due to (what looks like) a bug in libpng-1.2.4, we can't pass
  // NULL for the png_ptr arg
