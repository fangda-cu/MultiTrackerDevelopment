...
void RodRenderer::render()
{
    ...
    {
        m_palette[0] = Color( 255, 125, 75 );
        m_palette[1] = Color( 55, 125, 255 );
    }
...
void RodRenderer::drawSimpleRod()
{
    if ( m_rod.draw_cl == 0 )
    {
        ...
        for ( eit = m_rod.edges_begin(); eit != m_rod.edges_end(); ++eit )
        {
            ...
            EdgeVertexIterator evit = m_rod.ev_iter( *eit );
...
void RodRenderer::drawSmoothRod()
{
    ...
    for ( int j = 0; j < m_rod.ne(); ++j )
    {
        ...
        {
            if ( ( 0 <= k && k < ( slices + 3 ) / 4 ) || ( slices / 2 <= k && k < ( 3 * slices + 3 )
                    / 4 ) )
            {
                OpenGL::color( color1 );
            }
...
void RodRenderer::drawSmoothPartialRod( const int i_startVertex, const int i_endVertex,
        const Vec3d i_color )
{
    m_tube.buildTube();
