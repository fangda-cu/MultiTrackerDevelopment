...
int MKLLinearSolver::solve(VecXd& x, const VecXd& b)
{
    ...
    if (info < 0)
    {
        DebugStream(g_log, "") << "\033[31;1mWARNING IN MKLLinearSolver:\033[m Error in parameter " << -info
                << " in call to dgbsv" << '\n';
        return -1;
    }
    else if (info > 0)
    {
        DebugStream(g_log, "") << "\033[31;1mWARNING IN MKLLinearSolver:\033[m Factor U is singular, info code: " << info
                << '\n';
        return -1;
    }
    ...
    if( infnorm > 1.0e-6 )
    {
        DebugStream(g_log, "") << "\033[31;1mWARNING IN MKLLinearSolver:\033[m Large residual detected. ||residual||_{inf} = " << infnorm << '\n';
        return -1;
    }
#endif
