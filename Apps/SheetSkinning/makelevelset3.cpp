...
// find distance x0 is from segment x1-x2
static float point_segment_distance(const Vec3f &x0, const Vec3f &x1, const Vec3f &x2, float& s12)
{
  Vec3f dx(x2-x1);
  double m2=mag2(dx);
  // find parameter value of closest point on segment
  s12=(float)(dot(x2-x0, dx)/m2);
  if(s12<0){
    s12=0;
  }else if(s12>1){
    s12=1;
...
// find distance x0 is from triangle x1-x2-x3
static float point_triangle_distance(const Vec3f &x0, const Vec3f &x1, const Vec3f &x2, const Vec3f &x3, Vec3f& barycoords)
{
  // first find barycentric coordinates of closest point on infinite plane
  Vec3f x13(x1-x3), x23(x2-x3), x03(x0-x3);
  float m13=mag2(x13), m23=mag2(x23), d=dot(x13,x23);
  ...
  if(w23>=0 && w31>=0 && w12>=0){ // if we're inside the triangle
    ...
    if(w23>0) {// this rules out edge 2-3 for us
      float bary1, bary2;
      float opt1 = point_segment_distance(x0,x1,x2, bary1);
      float opt2 = point_segment_distance(x0,x1,x3, bary2);
      if(opt1 < opt2) {
        barycoords[0] = bary1;
        barycoords[1] = 1-bary1;
        barycoords[2] = 0;
        return opt1;
      }
      else {
      ...
      float bary1, bary2;
      float opt1 = point_segment_distance(x0,x1,x2, bary1);
      float opt2 = point_segment_distance(x0,x2,x3, bary2);
      if(opt1 < opt2) {
        barycoords[0] = bary1;
        barycoords[1] = 1-bary1;
        barycoords[2] = 0;
      ...
      float bary1, bary2;
      float opt1 = point_segment_distance(x0,x1,x3, bary1);
      float opt2 = point_segment_distance(x0,x2,x3, bary2);
      if(opt1 < opt2) {
        barycoords[0] = bary1;
        barycoords[1] = 0;
        barycoords[2] = 1-bary1;
        return opt1;
      }
      else {
        barycoords[0] = 0;
...
static void sweep(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
                  ...
                  int di, int dj, int dk, const Array3c& valid_cells)
{
   int i0, i1;
   ...
   for(int k=k0; k!=k1; k+=dk) for(int j=j0; j!=j1; j+=dj) for(int i=i0; i!=i1; i+=di){
      if(valid_cells(i,j,k) == 0)  //skip processing cells we've decided are too far.
        continue;
...
void make_distance_field3(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
                          const std::vector<double> &thicknesses,
                          const Vec3f &origin, float dx, int ni, int nj, int nk,
                          Array3f &phi, float cull_bound, const int exact_band)
{
  phi.resize(ni, nj, nk);
  phi.assign((ni+nj+nk)*dx); // upper bound on distance
  Array3i closest_tri(ni, nj, nk, -1);
  float cap = ((ni+nj+nk)*dx);
  Array3c valid_cell(ni,nj,nk, (char)0);
  ...
  //expand the list of valid cells (cells on the surface) by several layers (i.e. cells near the surface), 
  //and then only process those cells
  
  int expand_cell_count = (int)(cull_bound / dx)+1; //roughly where to stop processing cells
  std::cout << "Expand cell count:" << expand_cell_count << "\n";
  Array3c valid_temp(ni,nj,nk,(char)0);
  valid_temp = valid_cell;
  for(int iter = 0; iter < expand_cell_count; ++iter) {
    for(int k = 0; k < nk; ++k) for(int j = 0; j < nj; ++j) for(int i = 0; i < ni; ++i) {
      if(valid_cell(i,  j,   k) == 1) { //turn on all neighbours
        for(int offk = k-1; offk <= k+1; ++offk) for(int offj = j-1; offj <= j+1; ++offj) for(int offi = i-1; offi <= i+1; ++offi) {
          if(offk == 0 && offj == 0 && offi == 0) continue;
          if(offi < 0 ||offi >= ni || offj < 0 || offj >= nj || offk < 0 || offk >= nk)
            continue;

          valid_temp(offi,offj,offk) = 1;
        }
      }
    }
    valid_cell = valid_temp;
  }
  

  // and now we fill in the rest of the distances with fast sweeping
  for(unsigned int pass=0; pass<2; ++pass){
    sweep(tri, x, phi, closest_tri, origin, dx, +1, +1, +1, valid_cell);
    sweep(tri, x, phi, closest_tri, origin, dx, -1, -1, -1, valid_cell);
    sweep(tri, x, phi, closest_tri, origin, dx, +1, +1, -1, valid_cell);
    sweep(tri, x, phi, closest_tri, origin, dx, -1, -1, +1, valid_cell);
    sweep(tri, x, phi, closest_tri, origin, dx, +1, -1, +1, valid_cell);
  ...
  //postprocess to figure out thicknesses at each gridpoint
  for(int k = 0; k < nk; ++k) {
    for(int j = 0; j < nj; ++j) {
      for(int i = 0; i < ni; ++i) {
        if(valid_cell(i,j,k) == 0) {
          phi(i,j,k) = cull_bound; 
          continue;
        }
        Vec3f point(i*dx+origin[0], j*dx+origin[1], k*dx+origin[2]);
        int close_tri = closest_tri(i,j,k);
        unsigned int t0 = tri[close_tri][0], 
            t1 = tri[close_tri][1], 
            t2 = tri[close_tri][2];
        Vec3f v0 = x[t0], v1 = x[t1], v2 = x[t2];
