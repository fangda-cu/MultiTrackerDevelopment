...
MStatus WmSweeneyNode::compute(const MPlug& i_plug, MDataBlock& i_dataBlock) {
	...
	if (i_plug == ca_rodPropertiesSync) {
		...
		MObject strandVerticesObj =
				i_dataBlock.inputValue(ia_strandVertices).data();
		MFnVectorArrayData strandVerticesArrayData(strandVerticesObj, &status);
		CHECK_MSTATUS(status);
		...
		MVectorArray strandRootFrames =
				strandRootFramesArrayData.array(&status);
		CHECK_MSTATUS(status);
		...
		int numberOfVerticesPerStrand = i_dataBlock.inputValue(
				ia_verticesPerStrand).asInt();
		...
		if (m_currentTime == m_startTime) {
			...
			if (m_rodManager != NULL) {
				...
				for (size_t i = 0; i < m_rodManager->m_rods.size(); ++i) {
					...
					std::vector<Scalar> rest_lengths;
					// Adjust edge lengths and compute the resulting curl length
					for (ElasticRod::edge_iter eh =
							m_rodManager->m_rods[i]->edges_begin(); eh
							!= m_rodManager->m_rods[i]->edges_end(); ++eh) {
						rest_lengths.push_back(updated_edge_length);
						//cout << "WmSweeneyNode::compute::edges: idx = " << eh->idx() << " new edge length = " << updated_edge_length << " current edge length " << m_rodManager->m_rods[i]->getEdgeLength( *eh )  << endl;
						if (eh->idx() >= m_curlStart * (m_verticesPerRod - 1)) {
							curl_len += m_rodManager->m_rods[i]->getEdgeLength(
									*eh);
							curl_resolution++;
						}
					}
					...
					for (ElasticRod::vertex_iter vh =
							m_rodManager->m_rods[i]->vertices_begin(); vh
							!= m_rodManager->m_rods[i]->vertices_end(); ++vh) {
						assert(m_rodManager->m_rods[i]->m_bendingForce != NULL);
						...
						// TODO(sainsley) : verify this works for any curl start
						if (t > 0)
							curvature = m_curlRadius * curl_len
									/ curl_resolution;
						//if ( m_curlRadius != 0 )
						//{
						//  curvature /= ( m_curlRadius*m_curlRadius + m_curlPitch*m_curlPitch) ;
						...
						Scalar torsion = m_curlPitch * curl_len
								/ curl_resolution;
						//if ( m_curlPitch != 0 )
						//{
						//  torsion /= ( m_curlRadius*m_curlRadius + m_curlPitch*m_curlPitch) ;
						...
						m_rodManager->m_rods[i]->m_bendingForce->setKappaBar(
								*vh,
								Vec2d(curvature * cos(torsion * t),
										curvature * sin(torsion * t)));
						//cout << "WmSweeneyNode::compute::simulate: idx = " << vh->idx() << " parametric var = " << t << " curvature " <<  m_rodManager->m_rods[i]->m_bendingForce->getKappaBar(*vh) << " bending stiffness " <<  m_rodManager->m_rods[i]->m_bendingForce->getB(*vh) << " vertex mass " << m_rodManager->m_rods[i]->getVertexMass(vh->idx()) << endl;

						// grab edge out of current vertex and increment parametic length accordingly
						if (vh->idx() >= m_curlStart * (m_verticesPerRod) && vh
								!= m_rodManager->m_rods[i]->vertices_end()) {
							t += m_rodManager->m_rods[i]->getEdgeLength(j++)
									/ curl_len;
						}
		...
		if (m_rodManager != NULL) {
			double m_atol = powf(10,
					-i_dataBlock.inputValue(ia_atol).asDouble());
			double m_stol = powf(10,
					-i_dataBlock.inputValue(ia_stol).asDouble());
			double m_rtol = powf(10,
					-i_dataBlock.inputValue(ia_rtol).asDouble());
			double m_inftol = powf(10,
					-i_dataBlock.inputValue(ia_inftol).asDouble());
			int m_numLineSearchIters = i_dataBlock.inputValue(
					ia_numLineSearchIters).asInt();

			m_rodManager->updateSolverSettings(m_atol, m_stol, m_rtol,
					m_inftol, m_numLineSearchIters);
		}
	...
	return MS::kSuccess;
>>>>>>> rod base material frame FINALLY coming from barbershop
}

void WmSweeneyNode::initialiseCollisionMeshes(MDataBlock &i_data) {
	MStatus status;

	MArrayDataHandle inArrayH = i_data.inputArrayValue(ia_collisionMeshes,
			&status);
	CHECK_MSTATUS(status);
		...
		if (plug.isArray(&status)) {
			...
			if (indxPlug.isConnected(&status)) {
				...
				WmBunsenCollisionMeshNode
						* collisionMeshNode =
								(WmBunsenCollisionMeshNode*) collisionMeshNodeFn.userNode();

				TriangleMesh* triangleMesh = NULL;
				WmFigMeshController* figMeshController = NULL;

				collisionMeshNode->initialise(NULL, i, &triangleMesh,
						&figMeshController);

				// Now add the mesh to the rod manager
				m_rodManager->addCollisionMesh(triangleMesh,
						figMeshController->currentLevelSet(), figMeshController);
			} else {
				CHECK_MSTATUS(status);
			}
		}
...
void WmSweeneyNode::updateCollisionMeshes(MDataBlock& i_dataBlock) {
	...
	MArrayDataHandle inArrayH = i_dataBlock.inputArrayValue(ia_collisionMeshes,
			&status);
	CHECK_MSTATUS(status);
	size_t numMeshesConnected = inArrayH.elementCount();
...
void WmSweeneyNode::initialiseRodFromBarberShopInput(MDataBlock& i_dataBlock) {
	...
	cerr
			<< "initialiseRodFromBarberShopInput() - About to create rods from Barbershop input\n";
	...
	cerr
			<< "initialiseRodFromBarberShopInput() - Deleted and created a new WmSweeneyRodManager\n";

	if (m_strandVertices.length() == 0) {
		cerr
				<< "initialiseRodFromBarberShopInput() - no input strands so can't create any rods";
		return;
	}
	...
	// take its initial direction as a follicle angle
	unsigned int currentVertexIndex = 0;
	unsigned int numberOfStrands = m_strandVertices.length()
			/ m_numberOfVerticesPerStrand;

	cerr << "initialiseRodFromBarberShopInput() - m_strandVertices.length() = "
			<< m_strandVertices.length() << endl;
	cerr
			<< "initialiseRodFromBarberShopInput() - number of barberShop strands = "
			<< numberOfStrands << endl;
	cerr
			<< "initialiseRodFromBarberShopInput() - number of vertices per barberShop strand = "
			<< m_numberOfVerticesPerStrand << endl;

	vector<BASim::Vec3d> vertices;

	for (unsigned int inputStrandNumber = 0; inputStrandNumber
			< numberOfStrands; ++inputStrandNumber) {
		MVector direction = m_strandVertices[currentVertexIndex + 1]
				- m_strandVertices[currentVertexIndex];
		...
		//cout << "WmSweeneyNode::initialiseRodFromBarbershopInput:: create rod " << inputStrandNumber
			//			<< " direction " << direction << " m1 " << m1 << " m2 " << m2 << " m3 " << m3
				//		<< " dot " << m1.dot(m2) << " " << m1.cross(m2) << endl;

		constructRodVertices(vertices, direction,
				m_strandVertices[currentVertexIndex]);
	...
	//Set performancetuningparameters to pass through
	PerformanceTuningParameters perfParams;
	perfParams.m_enable_penalty_response = i_dataBlock.inputValue(
			ia_enablePenaltyResponse).asBool();
	perfParams.m_implicit_thickness = i_dataBlock.inputValue(
			ia_implicitThickness).asDouble();
	perfParams.m_implicit_stiffness = i_dataBlock.inputValue(
			ia_implicitStiffness).asDouble();
	perfParams.m_inextensibility_threshold = i_dataBlock.inputValue(
			ia_inextensibilityThreshold).asInt();
	perfParams.m_solver.m_max_iterations = i_dataBlock.inputValue(
			ia_maxNumOfSolverIters).asInt();
	perfParams.m_collision.m_max_iterations = i_dataBlock.inputValue(
			ia_maxNumOfCollisionIters).asInt();
	perfParams.m_enable_explosion_detection = i_dataBlock.inputValue(
			ia_enableExplosionDetection).asBool();
	perfParams.m_explosion_damping = i_dataBlock.inputValue(
			ia_explosionDampening).asDouble();
	perfParams.m_explosion_threshold = i_dataBlock.inputValue(
			ia_explosionThreshold).asDouble();
	perfParams.m_stretching_threshold = i_dataBlock.inputValue(
			ia_stretchingThreshold).asDouble();
	perfParams.m_solver.m_in_case_of
			= (BASim::FailureMode::ResponseSeverity) i_dataBlock.inputValue(
					ia_solverFailure).asInt();
	perfParams.m_collision.m_in_case_of
			= (BASim::FailureMode::ResponseSeverity) i_dataBlock.inputValue(
					ia_collisionFailure).asInt();
	perfParams.m_explosion.m_in_case_of
			= (BASim::FailureMode::ResponseSeverity) i_dataBlock.inputValue(
					ia_explosionFailure).asInt();
	perfParams.m_stretching.m_in_case_of
			= (BASim::FailureMode::ResponseSeverity) i_dataBlock.inputValue(
					ia_stretchingFailure).asInt();
	perfParams.m_solver.m_max_substeps = i_dataBlock.inputValue(
			ia_maxNumSolverSubsteps).asInt();
	perfParams.m_collision.m_max_substeps = i_dataBlock.inputValue(
			ia_maxNumCollisionSubsteps).asInt();
	perfParams.m_explosion.m_max_substeps = i_dataBlock.inputValue(
			ia_maxNumExplosionSubsteps).asInt();
	perfParams.m_stretching.m_max_substeps = i_dataBlock.inputValue(
			ia_maxNumStretchingSubsteps).asInt();
	double m_atol = powf(10, -i_dataBlock.inputValue(ia_atol).asDouble());
	double m_stol = powf(10, -i_dataBlock.inputValue(ia_stol).asDouble());
	double m_rtol = powf(10, -i_dataBlock.inputValue(ia_rtol).asDouble());
	double m_inftol = powf(10, -i_dataBlock.inputValue(ia_inftol).asDouble());
	int m_numLineSearchIters =
			i_dataBlock.inputValue(ia_numLineSearchIters).asInt();

	cerr
			<< "initialiseRodFromBarberShopInput() - About to initialise simulation\n";
	m_rodManager->initialiseSimulation(1 / 24.0, m_startTime, perfParams,
			m_atol, m_stol, m_rtol, m_inftol, m_numLineSearchIters);
	cerr
			<< "initialiseRodFromBarberShopInput() - Simulation initialised at time "
			<< m_startTime << endl;
}

void WmSweeneyNode::constructRodVertices(vector<BASim::Vec3d>& o_rodVertices,
	...
	for (int v = 0; v < m_verticesPerRod; ++v) {
		...
		o_rodVertices.push_back(
				BASim::Vec3d(currentVertex.x, currentVertex.y, currentVertex.z));
...
void WmSweeneyNode::compute_oa_simulatedNurbs(const MPlug& i_plug,
	...
	MArrayDataHandle simulatedNurbsArrayHandle = i_dataBlock.outputArrayValue(
			oa_simulatedNurbs, &status);
	CHECK_MSTATUS(status);

	MArrayDataBuilder simulatedNurbsArrayDataBuilder(&i_dataBlock,
			oa_simulatedNurbs, (unsigned int) m_rodManager->numberOfRods(),
			&status);
	CHECK_MSTATUS(status);
	...
	for (size_t r = 0; r < m_rodManager->numberOfRods(); ++r) {
		...
		MObject nurbsObj = nurbsFn.createWithEditPoints(nurbsEditPoints, 1,
				MFnNurbsCurve::kOpen, false /*not 2d*/, false /*not rational*/,
				true /*uniform params*/, nurbsDataObj, &status);
		CHECK_MSTATUS(status);

		MDataHandle simulatedNurbsHandle =
				simulatedNurbsArrayDataBuilder.addElement((unsigned int) r,
						&status);
		CHECK_MSTATUS(status);
...
void WmSweeneyNode::draw(M3dView& i_view, const MDagPath& i_path,
		M3dView::DisplayStyle i_style, M3dView::DisplayStatus i_status) {
	MStatus status;
...
MStatus WmSweeneyNode::connectionMade(const MPlug & plug,
...
MStatus WmSweeneyNode::connectionBroken(const MPlug & plug,
		const MPlug & otherPlug, bool asSrc) {
	MStatus stat;
	MStatus retVal(MS::kUnknownParameter);
...
/*static */MStatus WmSweeneyNode::addNumericAttribute(MObject& i_attribute,
		MString i_longName, MString i_shortName, MFnNumericData::Type i_type,
		double i_defaultValue, bool i_isInput, bool i_isArray) {
	// Creates a numeric attribute with default attributes
	MStatus stat = MS::kSuccess;

	MFnNumericAttribute nAttr;
	i_attribute = nAttr.create(i_longName, i_shortName, i_type, i_defaultValue,
			&stat);
	if (!stat) {
...
/* static */MStatus WmSweeneyNode::initialize() {
	...
	addNumericAttribute(ca_rodPropertiesSync, "rodPropertiesSync", "rps",
			MFnNumericData::kBoolean, false, false);
	...
	addNumericAttribute(ia_startTime, "startTime", "stt",
			MFnNumericData::kDouble, 1.0, true);
...

	{
		...
		ia_length = numericAttr.create("length", "len",
				MFnNumericData::kDouble, 10.0, &status);
		CHECK_MSTATUS(status);
	...
	addNumericAttribute(ia_edgeLength, "edgeLength", "ele",
			MFnNumericData::kDouble, 1.0, true);
	...
	{
		MFnNumericAttribute numericAttr;
		ia_rodRadius = numericAttr.create("rodRadius", "ror",
				MFnNumericData::kDouble, 0.005, &status);
		CHECK_MSTATUS(status);
		CHECK_MSTATUS(numericAttr.setReadable(true));
		CHECK_MSTATUS(numericAttr.setWritable(true));
	...
	{
		MFnNumericAttribute numericAttr;
		ia_rodAspectRatio = numericAttr.create("rodAspectRatio", "roar",
		...
		if (!status) {
			status.perror(
					"attributeAffects ia_rodAspectRatio->ca_rodPropertiesSync");
			return status;
		}
	}
	...
	{
		MFnNumericAttribute numericAttr;
		ia_rodRotation = numericAttr.create("rodRotation", "rorot",
		...
		if (!status) {
			status.perror(
					"attributeAffects ia_rodRotation->ca_rodPropertiesSync");
			return status;
		}
	}
	...
	{
		MFnNumericAttribute numericAttr;
		ia_curlRadius = numericAttr.create("curlTightness", "crlrad",
				MFnNumericData::kDouble, 0.0, &status);
		CHECK_MSTATUS(status);
		CHECK_MSTATUS(numericAttr.setReadable(true));
		CHECK_MSTATUS(numericAttr.setWritable(true));
		...
		if (!status) {
			status.perror(
					"attributeAffects ia_curlRadius->ca_rodPropertiesSync");
			return status;
		}
	}
	...
	{
		MFnNumericAttribute numericAttr;
		ia_curlPitch = numericAttr.create("curlSpacing", "crlptch",
				MFnNumericData::kDouble, 0.5, &status);
		CHECK_MSTATUS(status);
		CHECK_MSTATUS(numericAttr.setReadable(true));
		CHECK_MSTATUS(numericAttr.setWritable(true));
		CHECK_MSTATUS(numericAttr.setMin(0.0));
		...
		if (!status) {
			status.perror("attributeAffects ia_curlPitch->ca_rodPropertiesSync");
			return status;
		}
	}
	...
	{
		MFnNumericAttribute numericAttr;
		ia_curlStart = numericAttr.create("curlStart", "crlstrt",
				MFnNumericData::kDouble, 0.0, &status);
		CHECK_MSTATUS(status);
		CHECK_MSTATUS(numericAttr.setReadable(true));
		CHECK_MSTATUS(numericAttr.setWritable(true));
		...
		if (!status) {
			status.perror("attributeAffects ia_curlStart->ca_rodPropertiesSync");
			return status;
		}
	}

	addNumericAttribute(ia_rodDamping, "rodDamping", "roddamp",
			MFnNumericData::kBoolean, true, true);
	status = attributeAffects(ia_rodDamping, ca_rodPropertiesSync);
	if (!status) {
		status.perror("attributeAffects ia_rodDamping->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_rodPitch, "rodPitch", "rop",
			MFnNumericData::kDouble, 0.5, true);
	status = attributeAffects(ia_rodPitch, ca_rodPropertiesSync);
	if (!status) {
		status.perror("attributeAffects ia_rodPitch->ca_rodPropertiesSync");
		return status;
	}
	...
	if (!status) {
		status.perror(
				"attributeAffects ia_strandVertices->ca_rodPropertiesSync");
		return status;
	}
	...
	if (!status) {
		status.perror(
				"attributeAffects ia_strandRootFrames->ca_rodPropertiesSync");
		return status;
	}
	...
	if (!status) {
		status.perror("attributeAffects ia_time->oa_simulatedNurbs");
		return status;
	}
	status = attributeAffects(ca_rodPropertiesSync, oa_simulatedNurbs);
	if (!status) {
		status.perror(
				"attributeAffects ca_rodPropertiesSync->oa_simulatedNurbs");
		return status;
	}

	{
		MFnNumericAttribute nAttr;
		ia_collisionMeshes = nAttr.create("collisionMeshes", "com",
		...
		if (!status) {
			status.perror("addAttribute ia_collisionMeshes");
			return status;
		}
	}
	...
	if (!status) {
		status.perror(
				"attributeAffects ia_collisionMeshes->ca_rodPropertiesSync");
		return status;
	}

	//Solver settings
	addNumericAttribute(ia_stol, "stol", "stl", MFnNumericData::kDouble, 99,
			true);
	status = attributeAffects(ia_stol, ca_rodPropertiesSync);
	if (!status) {
		status.perror("attributeAffects ia_stol->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_atol, "atol", "atl", MFnNumericData::kDouble, 8,
			true);
	status = attributeAffects(ia_atol, ca_rodPropertiesSync);
	if (!status) {
		status.perror("attributeAffects ia_atol->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_rtol, "rtol", "rtl", MFnNumericData::kDouble, 99,
			true);
	status = attributeAffects(ia_rtol, ca_rodPropertiesSync);
	if (!status) {
		status.perror("attributeAffects ia_rtol->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_inftol, "inftol", "itl", MFnNumericData::kDouble, 8,
			true);
	status = attributeAffects(ia_inftol, ca_rodPropertiesSync);
	if (!status) {
		status.perror("attributeAffects ia_inftol->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_numLineSearchIters, "numLineSearchIters", "nlsi",
			MFnNumericData::kInt, 2, true);
	status = attributeAffects(ia_numLineSearchIters, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_numLineSearchIters->ca_rodPropertiesSync");
		return status;
	}

	//General parameters
	addNumericAttribute(ia_enablePenaltyResponse, "enablePenaltyResponse",
			"epr", MFnNumericData::kBoolean, true, true);
	status = attributeAffects(ia_enablePenaltyResponse, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_enablePenaltyResponse->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_implicitThickness, "implicitThickness", "imt",
			MFnNumericData::kDouble, 0.10, true);
	status = attributeAffects(ia_implicitThickness, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_implicitThickness->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_implicitStiffness, "implicitStiffness", "ims",
			MFnNumericData::kDouble, 1.0, true);
	status = attributeAffects(ia_implicitStiffness, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_implicitStiffness->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_inextensibilityThreshold,
			"inextensibilityThreshold", "ixf", MFnNumericData::kInt, 0, true);
	status
			= attributeAffects(ia_inextensibilityThreshold,
					ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_inextensibilityThreshold->ca_rodPropertiesSync");
		return status;
	}

	//Failure  Detection
	addNumericAttribute(ia_maxNumOfSolverIters, "maxNumOfSolverIters", "mnsi",
			MFnNumericData::kInt, 250, true);
	status = attributeAffects(ia_maxNumOfSolverIters, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_maxNumOfSolverIters->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_maxNumOfCollisionIters, "maxNumOfCollisionIters",
			"mnci", MFnNumericData::kInt, 0, true);
	status = attributeAffects(ia_maxNumOfCollisionIters, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_maxNumOfCollisionIters->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_enableExplosionDetection,
			"enableExplosionDetection", "eex", MFnNumericData::kBoolean, true,
			true);
	status
			= attributeAffects(ia_enableExplosionDetection,
					ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_enableExplosionDetection->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_explosionDampening, "explosionDampening", "exd",
			MFnNumericData::kDouble, 100.0, true);
	status = attributeAffects(ia_explosionDampening, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_iexplosionDampening->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_explosionThreshold, "explosionThreshold", "ext",
			MFnNumericData::kDouble, 0.5, true);
	status = attributeAffects(ia_explosionThreshold, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_explosionThreshold->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_stretchingThreshold, "stretchingThreshold", "sxt",
			MFnNumericData::kDouble, 2.0, true);
	status = attributeAffects(ia_stretchingThreshold, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_stretchingThreshold->ca_rodPropertiesSync");
		return status;
	}

	{
		MFnEnumAttribute enumAttrFn;
		ia_solverFailure = enumAttrFn.create("ifSolverStillFails", "svf",
				(short) FailureMode::IgnoreError, &status);
		CHECK_MSTATUS(status);
		...
		enumAttrFn.addField("Halt simulation",
				(short) FailureMode::HaltSimulation);
	...
	addNumericAttribute(ia_maxNumSolverSubsteps, "maxNumSolverSubsteps",
			"mnss", MFnNumericData::kInt, 0, true);
	status = attributeAffects(ia_maxNumSolverSubsteps, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_maxNumSolverSubsteps->ca_rodPropertiesSync");
		return status;
	}

	{
		MFnEnumAttribute enumAttrFn;
		ia_collisionFailure = enumAttrFn.create("ifCollisionStillFails", "clf",
				(short) FailureMode::IgnoreError, &status);
		CHECK_MSTATUS(status);
		enumAttrFn.addField("Ignore error", (short) FailureMode::IgnoreError);
		enumAttrFn.addField("Kill the rod", (short) FailureMode::KillTheRod);
	...
	if (!status) {
		status.perror(
				"attributeAffects ia_collisionFailure->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_maxNumCollisionSubsteps, "maxNumCollisionSubsteps",
			"mncs", MFnNumericData::kInt, 0, true);
	status = attributeAffects(ia_maxNumCollisionSubsteps, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_maxNumCollisionSubsteps->ca_rodPropertiesSync");
		return status;
	}

	{
		MFnEnumAttribute enumAttrFn;
		ia_explosionFailure = enumAttrFn.create("ifExplosionStillFails", "exf",
				(short) FailureMode::IgnoreError, &status);
		CHECK_MSTATUS(status);
		enumAttrFn.addField("Ignore error", (short) FailureMode::IgnoreError);
		enumAttrFn.addField("Kill the rod", (short) FailureMode::KillTheRod);
	...
	if (!status) {
		status.perror(
				"attributeAffects ia_explosionFailure->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_maxNumExplosionSubsteps, "maxNumExplosionSubsteps",
			"mnes", MFnNumericData::kInt, 7, true);
	status = attributeAffects(ia_maxNumExplosionSubsteps, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_maxNumExplosionSubsteps->ca_rodPropertiesSync");
		return status;
	}

	{
		MFnEnumAttribute enumAttrFn;
		ia_stretchingFailure = enumAttrFn.create("ifStretchingStillFails",
				"sxf", (short) FailureMode::KillTheRod, &status);
		CHECK_MSTATUS(status);
		enumAttrFn.addField("Ignore error", (short) FailureMode::IgnoreError);
		enumAttrFn.addField("Kill the rod", (short) FailureMode::KillTheRod);
		enumAttrFn.addField("Halt simulation",
	...
	if (!status) {
		status.perror(
				"attributeAffects ia_stretchingFailure->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_maxNumStretchingSubsteps,
			"maxNumStretchingSubsteps", "mnts", MFnNumericData::kInt, 0, true);
	status
			= attributeAffects(ia_maxNumStretchingSubsteps,
					ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_maxNumStretchingSubsteps->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_verticesPerStrand, "verticesPerStrand", "vps",
			MFnNumericData::kInt, 12, true);
	status = attributeAffects(ia_verticesPerStrand, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects verticesPerStrand->ca_rodPropertiesSync");
		return status;
	}

	addNumericAttribute(ia_verticesPerRod, "verticesPerRod", "cpr",
			MFnNumericData::kInt, 10, true);
	status = attributeAffects(ia_verticesPerRod, ca_rodPropertiesSync);
	if (!status) {
		status.perror(
				"attributeAffects ia_verticesPerRod->ca_rodPropertiesSync");
		return status;
	}
