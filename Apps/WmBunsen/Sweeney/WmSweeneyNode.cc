        ...
        else
        {
            if (m_rodManager != NULL)
            {
                ...
                for (size_t i = 0; i < m_rodManager->m_rods.size(); ++i)
                {
                    ...
                    for (ElasticRod::edge_iter eh = current_rod->edges_begin(); eh != current_rod->edges_end(); ++eh)
                    {
                    ...
                    if (radius_a != current_rod->radiusA(0) || radius_b != current_rod->radiusB(0))
                    {
                        current_rod->setRadius(radius_a, radius_b);
                        update_rod = true;
                        cout << "NONEQ RAD " << current_rod->radiusA(0) << " " << radius_a << current_rod->radiusB(0) << " "
                                << radius_b << endl;
                    }
                    ...
                    for (ElasticRod::vertex_iter vh = current_rod->vertices_begin(); vh != current_rod->vertices_end(); ++vh)
                    {
                        ...
                        if (current_rod->m_twistingForce->getUndeformedTwist(*vh) != torsion)
                        {
                            ...
                            update_rod = true;
                            cout << "NONEQ TWIST " << current_rod->m_twistingForce->getUndeformedTwist(*vh) << " " << torsion
                                    << endl;
                        }
...
void WmSweeneyNode::initialiseRodFromBarberShopInput(MDataBlock& i_dataBlock)
{
    ...
    for (unsigned int inputStrandNumber = 0; inputStrandNumber < numberOfStrands; ++inputStrandNumber)
    {
        ...
        if (m_strandRootFrames.length() != 0)
        {
            ...
            BASim::Vec3d m1 = Vec3d(m_strandRootFrames[3 * inputStrandNumber].x, m_strandRootFrames[3 * inputStrandNumber].y,
...
/* static */MStatus WmSweeneyNode::initialize()
{
...

    {
        ...
        if (!status)
        {
            status.perror("attributeAffects ia_curlRadius->ca_rodPropertiesSync");
            return status;
        }
    }
...

    {
        ...
        ia_strandVertices = tAttr.create("strandVertices", "sve", MFnData::kVectorArray, &status);
        CHECK_MSTATUS(status);
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_strandVertices->ca_rodPropertiesSync");
        return status;
    }

    {
        MFnTypedAttribute tAttr;
        ia_strandRootFrames = tAttr.create("strandRootFrames", "srf", MFnData::kVectorArray, &status);
        CHECK_MSTATUS(status);
        CHECK_MSTATUS(tAttr.setReadable(false));
        CHECK_MSTATUS(tAttr.setWritable(true));
        CHECK_MSTATUS(tAttr.setStorable(false));
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_strandRootFrames->ca_rodPropertiesSync");
        return status;
    }

    {
        MFnTypedAttribute tAttr;
        oa_simulatedNurbs = tAttr.create("simulatedNurbs", "sns", MFnData::kNurbsCurve, &status);
        CHECK_MSTATUS(status);
    ...
    if (!status)
    {
        status.perror("attributeAffects ca_rodPropertiesSync->oa_simulatedNurbs");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_collisionMeshes->ca_rodPropertiesSync");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_numLineSearchIters->ca_rodPropertiesSync");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_enablePenaltyResponse->ca_rodPropertiesSync");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_implicitThickness->ca_rodPropertiesSync");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_implicitStiffness->ca_rodPropertiesSync");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_levelsetSubsampling->ca_rodPropertiesSync");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("attributeAffects ia_inextensibilityThreshold->ca_rodPropertiesSync");
        return status;
    }
