...
WmFigCombContext* WmFigCombContext::activeContext = NULL;
MString WmFigCombContext::typeName = "wmFigCombContext";

WmFigCombContext::WmFigCombContext() : m_helpString( "Click or drag to select rods" ),
 m_figRodNode( NULL )
{        
    setTitleString( "Tool to comb Figaro rods" );

    // Tell the context which XPM to use so the tool can properly
    // be a candidate for the 6th position on the mini-bar.
    setImage( "WmFigCombContext.xpm", MPxContext::kImage1 );
}

void WmFigCombContext::getClassName( MString& name ) const 
{
    name = typeName;
}

void WmFigCombContext::toolOnSetup(MEvent &)
{
    MStatus stat;
    setHelpString( m_helpString );
}

void WmFigCombContext::toolOffCleanup()
{
}

MStatus WmFigCombContext::doPress( MEvent& i_event ) 
{    
    MStatus stat;

    i_event.getPosition( m_xMouseStart, m_yMouseStart );

    m_toolCommand = (WmFigCombToolCommand*) newToolCommand();

    return MS::kSuccess;
}

MStatus WmFigCombContext::doDrag( MEvent& i_event ) 
{
    MStatus stat;
...
bool WmFigCombContext::findRodNode()
{
    MStatus status;
    ...
    if ( selectionList.length() == 1 )
    {
        ...
        MFnDagNode rodDagNodeFn( rodNodeDagPath.child( 0, &status ), &status );
        if ( status )
        {
            status = rodDagNodeFn.getPath( childPath );
            CHECK_MSTATUS( status );
            childPath.extendToShape();
        
            MFnDependencyNode nodeFn( childPath.node( &status ) );
            CHECK_MSTATUS( status );
        ...
        {
            MFnDependencyNode nodeFn( rodNodeDagPath.node( &status ) );
            CHECK_MSTATUS( status );
        
            if ( nodeFn.typeName() == WmFigRodNode::typeName )
            {
                //    rodNodeObj = nodeFn.node();
                rodNodeObj = rodNodeDagPath.node( &status );
            }
            else
                foundRodNode = false;
        }
    ...
    m_figRodNodeMObject = rodNodeObj;
    m_figRodNode = rodNode;

    return true;
}

void WmFigCombContext::closestPointOnLine( const MPoint& i_pPt,
    const MPoint& i_aPt, const MPoint& i_bPt, MPoint& o_xPt, bool i_segment )
{
    MVector v1 = i_pPt - i_aPt;
    MVector v2 = i_bPt - i_aPt;
    double cosine = ( v1 * v2 ) / ( v1.length() * v2.length() );
    double proj = cosine * v1.length();
    double t = proj / v2.length();
    if ( i_segment ) clamp( t, 0.0, 1.0 );
    o_xPt = MPoint(  (1.0-t) * MVector( i_aPt )  +  t * MVector( i_bPt )  );
} 

void WmFigCombContext::applyForceToRods()
{
    // As this is a prototype, we don't care about speed.
    // Simply check every rod vertex in screen space and find
    // a set of vertices that need force applied to them.

    findRodNode();

    if ( m_figRodNode == NULL )
    {
        return;
    }

    //short xScreenNew = xScreen + short( float( dxMouseSmooth ) );
    ...
    cerr << "start = " << m_xMouseStart << ", " << m_yMouseStart << endl;
    cerr << "end = " << m_xMouse << ", " << m_yMouse << endl;

    MVector forceVector( m_xMouse - m_xMouseStart, m_yMouse - m_yMouseStart, 0 );
    forceVector.normalize();

    // scale by arbitrary size...
    forceVector *= 10.0;
    ...
    for ( int r=0; r<numberOfRods; ++r )
    {
        ...
        for ( int v=0; v<elasticRod->nv(); ++v )
        {
            ...
            if ( view.worldToView( vertex, screenX, screenY, &status ) )
            {
                // Point was not clipped
                MVector differenceVector( m_xMouse - screenX, m_yMouse - screenY, 0 );
                
                if ( differenceVector.length() < 50 )
                {
                    cerr << "rod " << r << ", vertex " << v << " is within distance!\n";

                    // Work out where to move this point to.
                    
                    screenX += forceVector[ 0 ];
                    screenY += forceVector[ 1 ];
                    
                    MPoint nearNewPt, farNewPt;
                    view.viewToWorld( screenX, screenY, nearNewPt, farNewPt );
        ...
        elasticRod->updateProperties();
    }

    // Now make the rod evaluate to fix the position of all the vertices that were not moved
    // by the user.
    makeRodNodeEvaluate();
}

void WmFigCombContext::makeRodNodeEvaluate()
{
    MStatus status;

    MFnDependencyNode rodNodeDepFn( m_figRodNodeMObject );
    
    MPlug figaroNodePlug( m_figRodNodeMObject, WmFigRodNode::ia_massDamping );

 //   MPlugArray connectedPlugs;

  // figaroNodePlug.connectedTo( connectedPlugs, true, false, &status );
  //  CHECK_MSTATUS( status );

  /*  if ( connectedPlugs.length() == 0 )
    {
        MGlobal::displayWarning( "Problem finding Figaro node, cannot force evaluation." );
        return;
    }

   MObject figaroNode = connectedPlugs[ 0 ].node( &status );
    CHECK_MSTATUS( status );

    MFnDependencyNode figaroNodeFn( figaroNode, &status );
    CHECK_MSTATUS( status );

    MPlug syncPlug = figaroNodeFn.findPlug( WmBunsenNode::ia_massDamping, true, &status );
    CHECK_MSTATUS( status );*/

    bool b;
    figaroNodePlug.getValue( b );
    figaroNodePlug.getValue( b );
}

MStatus WmFigCombContext::doRelease( MEvent& i_event ) 
{
    MStatus stat;
...
GLint WmFigCombContext::findRodsUsingOpenGLSelection( const double i_centreX, const double i_centreY,
...
MStatus WmFigCombContext::doEnterRegion( MEvent & )
{
    return setHelpString( m_helpString );
}


MStatus WmFigCombContext::drawMarqueeSelectBox() const
{
    M3dView view = M3dView::active3dView();
    view.refresh( true, true );
...
const float* WmFigCombContext::projectionMatrix()
{
    MStatus status;
...
const float* WmFigCombContext::modelViewMatrix()
{
    MStatus status;
