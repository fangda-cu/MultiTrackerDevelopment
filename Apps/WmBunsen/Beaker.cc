...
void Beaker::addRodsToWorld( int i_rodGroupIndex, WmFigRodGroup* i_rodGroup ,double startTime)
{
  ...
  //  cerr << "about to add rods to world, dt = " << getDt() << endl;
    ...
    for ( CollisionMeshDataHashMap::iterator cmItr = m_collisionMeshDataHashMap.begin();
                                             cmItr != m_collisionMeshDataHashMap.end(); ++cmItr )
    {
    ...
    m_bridsonStepper = new BridsonStepper( m_rods, m_triangleMeshes, m_scriptingControllers, 
                                           m_rodTimeSteppers, 1.0/24.0, startTime);
