...
//  }
  ...
  if (progress_indicator && current_problem->dynamicsPropsLoaded()) std::cout << "\rtime = " << current_problem->getTime() << std::flush;
}
...
void serializeScene( const std::string& output_file )
{
  ...
  if( !of.is_open() ) 
  {
    std::cerr << "\033[31;1mWARNING IN BASIMULATOR:\033[m Failed to open file for scene serialization: " << output_file << ". No output saved." << std::endl;
    return;
  }
...
void resumeSerializedScene( const std::string& binary_file_name )
{
  ...
  if( !ifs.is_open() ) 
  {
    std::cerr << "\033[31;1mERROR IN BASIMULATOR:\033[m Failed to resume from serialized scene: " << binary_file_name << ". Exiting." << std::endl;
    exit(1);
  }
...
void RunProblem(int argc, char** argv)
{
  ...
  if (render) {
    ...
    if( g_sim_breakpoints->size() == 0 || g_sim_breakpoints->back() != std::numeric_limits<double>::infinity() ) g_sim_breakpoints->push(std::numeric_limits<double>::infinity());
...
int main(int argc, char** argv)
{
  ...
  if( g_resume_file == std::string("") ) current_problem->BaseSetup(argc, argv);
  else resumeSerializedScene( g_resume_file );
