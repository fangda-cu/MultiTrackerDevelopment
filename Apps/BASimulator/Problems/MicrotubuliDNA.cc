...
void MicrotubuliDNA::AtEachTimestep()
{
  ...
  Vec3d constraintForce0 = force.segment<3>(m_rods[0]->vertIdx(fixed0,0));
  Vec3d constraintForce1 = force.segment<3>(m_rods[0]->vertIdx(fixed1,0)); 
  
  std::cout << "F0: " << constraintForce0 << std::endl;
  std::cout << "F1: " << constraintForce1 << std::endl;
}
