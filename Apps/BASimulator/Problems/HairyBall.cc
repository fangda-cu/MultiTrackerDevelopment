...
SphereRotator::SphereRotator( TriangleMesh& mesh, std::vector<ElasticRod*>& rods, double time, double dt )
...
void SphereRotator::transformTriangleObject( TriangleMesh& tri_mesh, const Mat3d& transformation )
{
...
void SphereRotator::transformRodRoot( ElasticRod* rod, const Mat3d& transformation )
{
...
void HairyBall::Setup()
{
  ...
  m_rotator = new SphereRotator(*tri_mesh,m_rods,getTime(),GetScalarOpt("dt"));
...
void HairyBall::serialize( std::ofstream& of )
{
...
void HairyBall::resumeFromfile( std::ifstream& ifs )
{
  ...
  m_rotator = new SphereRotator(*m_tri_meshes[0],m_rods,getTime(),GetScalarOpt("dt"));
  m_scripting_controllers.push_back(m_rotator);
