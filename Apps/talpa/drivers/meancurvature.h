...
public:
    ...
    ///
    static double mixed_area( size_t vertex_index, size_t triangle_index, const ElTopo::SurfTrack& surf );
    
    /// Compute mean curvature times normal at a vertex
    ///
    static void vertex_mean_curvature_normal( size_t vertex_index, const ElTopo::SurfTrack& surf, ElTopo::Vec3d& out );
    
    /// Compute MC * normal at a vertex and return the sum of weights used (for computing the time step restriction)
    ///
    static void vertex_mean_curvature_normal( size_t vertex_index, const ElTopo::SurfTrack& surf, ElTopo::Vec3d& out, double& weight_sum );
    ...
    /// 
    double compute_l1_error( const ElTopo::SurfTrack& surf ) const;
    
    /// Compute the maximum distance from vertices to analytic surface
    /// 
    double compute_inf_error( const ElTopo::SurfTrack& surf ) const;
