...
public:
    ...
    inline       Vec3d  get_newposition( const Vec3d & pos, size_t ref_vert ) const; // PBC: this returns the position of one instance (out of the infinitely many instances) of position pos that's closest to the position of the base instance of vertex ref_vert
    inline       Vec3d  get_newposition( const Vec3d & pos, const Vec3d & ref_pos ) const; // PBC: this returns the position of one instance (out of the infinitely many instances) of position pos that's closest to the position ref_pos
...
{
    return get_newposition(get_newposition(index), get_newposition(ref_vert));
}

inline Vec3d DynamicSurface::get_newposition( size_t index, const Vec3d & ref_pos ) const
{
    return get_newposition(get_newposition(index), ref_pos);
}

inline Vec3d DynamicSurface::get_newposition( const Vec3d & pos, size_t ref_vert ) const
{
    return get_newposition(pos, get_newposition(ref_vert));
}

inline Vec3d DynamicSurface::get_newposition( const Vec3d & pos, const Vec3d & ref_pos ) const
{
    return pos - get_domain_offset(pos + (Vec3d(PBC_DOMAIN_SIZE_X, PBC_DOMAIN_SIZE_Y, PBC_DOMAIN_SIZE_Z) * 0.5 - ref_pos));
}
