                                                                    ...
                                                                    size_t vertex_d )
{
    ...
    
    {
        ...
        // do one big query into the broad phase for all 4 new triangles
        minmax( new_vertex_position, new_vertex_smooth_position, 
               m_surf.get_position( vertex_a ), m_surf.get_position( vertex_b ), m_surf.get_position( vertex_c ), m_surf.get_position( vertex_d ),
...
bool EdgeSplitter::split_edge_pseudo_motion_introduces_intersection2( const Vec3d& new_vertex_position, 
  const Vec3d& new_vertex_smooth_position, 
  size_t edge,
  size_t vertex_a,
  size_t vertex_b,
...
{   
    ...
    // Only split edges incident on 2 triangles
    ...
    Vec3st newtri0(vertex_a, vertex_e, vertex_c);
    Vec3st newtri1(vertex_c, vertex_e, vertex_b);
    Vec3st newtri2(vertex_d, vertex_b, vertex_e);
    Vec3st newtri3(vertex_d, vertex_e, vertex_a);
    size_t newtri0_id = m_surf.add_triangle( newtri0 );
    size_t newtri1_id = m_surf.add_triangle( newtri1 );
    size_t newtri2_id = m_surf.add_triangle( newtri2 );
    size_t newtri3_id = m_surf.add_triangle( newtri3 );      
    ...
    split.m_deleted_tris.push_back(tri0);
    split.m_deleted_tris.push_back(tri1);
    split.m_created_tris.push_back(newtri0_id);
    split.m_created_tris.push_back(newtri1_id);
    split.m_created_tris.push_back(newtri2_id);
    split.m_created_tris.push_back(newtri3_id);
    split.m_created_tri_data.push_back(newtri0);
    split.m_created_tri_data.push_back(newtri1);
    split.m_created_tri_data.push_back(newtri2);
    split.m_created_tri_data.push_back(newtri3);
...
bool EdgeSplitter::split_edge2( size_t edge )
{   

  g_stats.add_to_int( "EdgeSplitter:edge_split_attempts", 1 );

  assert( edge_is_splittable2(edge) );
  ...
  if ( use_smooth_point == false )
  {
    ...
    if ( split_edge_pseudo_motion_introduces_intersection2( new_vertex_position, 
      new_vertex_smooth_position, 
      edge, 
      vertex_a, 
      vertex_b, 
...
bool EdgeSplitter::edge_is_splittable2( size_t edge_index )
{
...
bool EdgeSplitter::large_angle_split_pass2()
{
  ...
  for ( size_t e = 0; e < mesh.m_edges.size(); ++e )
  {
    ...
    if ( !edge_is_splittable2(e) ) { continue; }
...
bool EdgeSplitter::split_pass()
{
    ...
    {    
        if ( !edge_is_splittable2(i) ) { continue; }
    ...
    for ( ; iter != sortable_edges_to_try.rend(); ++iter )
    {
        ...
        if ( !edge_is_splittable2(longest_edge) ) { continue; }
    ...
    bool large_angle_split_occurred = large_angle_split_pass2();
