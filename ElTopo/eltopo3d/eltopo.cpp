                               ...
                               struct ElTopoMesh* outputs )
{
    ...
    {
        ts.push_back( ElTopo::Vec3st( inputs->triangles[3*i], inputs->triangles[3*i + 1], inputs->triangles[3*i + 2] ) );
    }
    ...
    defrag_info->num_vertex_changes = ElTopo::to_int(surface_tracker.m_vertex_change_history.size());
    ...
    for ( int i = 0; i < defrag_info->num_vertex_changes; ++i )
    {
        ...
        defrag_info->vertex_index[i] = ElTopo::to_int(surface_tracker.m_vertex_change_history[i].m_vertex_index);
        defrag_info->split_edge[2*i+0] = ElTopo::to_int(surface_tracker.m_vertex_change_history[i].m_split_edge[0]);
        defrag_info->split_edge[2*i+1] = ElTopo::to_int(surface_tracker.m_vertex_change_history[i].m_split_edge[1]);
    }
    
    defrag_info->num_triangle_changes = ElTopo::to_int(surface_tracker.m_triangle_change_history.size());
    ...
    for ( int i = 0; i < defrag_info->num_triangle_changes; ++i )
    {
        ...
        defrag_info->triangle_index[i] = ElTopo::to_int(surface_tracker.m_triangle_change_history[i].m_triangle_index);
        defrag_info->new_tri[3*i+0] = ElTopo::to_int(surface_tracker.m_triangle_change_history[i].m_tri[0]);
        defrag_info->new_tri[3*i+1] = ElTopo::to_int(surface_tracker.m_triangle_change_history[i].m_tri[1]);
        defrag_info->new_tri[3*i+2] = ElTopo::to_int(surface_tracker.m_triangle_change_history[i].m_tri[2]);
    }
    
    
    defrag_info->defragged_triangle_map_size = ElTopo::to_int(surface_tracker.m_defragged_triangle_map.size());
    ...
    {
        defrag_info->defragged_triangle_map[2*i+0] = ElTopo::to_int(surface_tracker.m_defragged_triangle_map[i][0]);
        defrag_info->defragged_triangle_map[2*i+1] = ElTopo::to_int(surface_tracker.m_defragged_triangle_map[i][1]);
    }
    
    defrag_info->defragged_vertex_map_size = ElTopo::to_int(surface_tracker.m_defragged_vertex_map.size());
    ...
    {
        defrag_info->defragged_vertex_map[2*i+0] = ElTopo::to_int(surface_tracker.m_defragged_vertex_map[i][0]);
        defrag_info->defragged_vertex_map[2*i+1] = ElTopo::to_int(surface_tracker.m_defragged_vertex_map[i][1]);
    }
    ...
    {
        const ElTopo::Vec3d& pos = surface_tracker.get_position(i);
    ...
    for ( int i = 0; i < outputs->num_triangles; ++i )
    {
        ...
        outputs->triangles[3*i + 0] = ElTopo::to_int(curr_tri[0]);
        outputs->triangles[3*i + 1] = ElTopo::to_int(curr_tri[1]);
        outputs->triangles[3*i + 2] = ElTopo::to_int(curr_tri[2]);
    }
