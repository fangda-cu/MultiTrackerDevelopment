...
ElasticStrand::ElasticStrand(VecXd& dofs, const ParametersType& parameters) :
    m_parameters(parameters), m_geometry(dofs), m_dofsStorage(), m_newGeometry(m_dofsStorage), m_forceCachesUpToDate(false)
{
    resizeInternals();
    ...
    updateForceCaches(); // does nothing if the rest shape is the same as creation shape (see above).
}
...
// Compute everything, based on current geometry
void ElasticStrand::updateForceCaches()
{
    ...
    accumulateInternalForces(); // Updates internal energy, force, Jacobian.
...
{
    assert( m_geometry.m_framesUpToDate);

    for (IndexType vtx = 1; vtx < m_numVertices - 1; ++vtx)
        m_bendingMatrices[vtx] = computeBendingMatrix(vtx);

    ForceAccumulator<StretchingForce>::accumulate(*this, m_geometry);
    ForceAccumulator<BendingForce>::accumulate(*this, m_geometry);
    ForceAccumulator<TwistingForce>::accumulate(*this, m_geometry);
}

// We need the same methods working on m_newGeometry, computing only energy and force
// Then, we could reuse those when accepting new positions.
